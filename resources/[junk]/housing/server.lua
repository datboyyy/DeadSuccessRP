    ...
    MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {['@identifier'] = xPlayer.identifier}, function(result)
        ...
        if house.houseId == id then
            for k, v in pairs(Config.HouseSpawns) do
                if not v['taken'] then
                    TriggerClientEvent('loaf_housing:spawnHouse', xPlayer.source, v['coords'], json.decode(furniture))
...
RegisterServerEvent('loaf_housing:policeRaid')
AddEventHandler('loaf_housing:policeRaid', function(id)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
...
RegisterServerEvent('loaf_housing:visit')
AddEventHandler('loaf_housing:visit', function(id)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)

    for k, v in pairs(Config.HouseSpawns) do
        if not v['taken'] then
            TriggerClientEvent('loaf_housing:visitHouse', xPlayer.source, v['coords'], Config.Houses[id]['prop'], Config.Houses[id]['door'], k)
            v['taken'] = true
            return
        end
    end
...
RegisterServerEvent('loaf_housing:leaveVisit')
AddEventHandler('loaf_housing:leaveVisit', function(housespawn)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
...
AddEventHandler('loaf_housing:getOwned', function()
    ...
    MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {['@identifier'] = xPlayer.identifier}, function(result)
        if result[1]['house'] then
            ...
            MySQL.Async.execute("UPDATE users SET house=@house WHERE identifier=@identifier", {['@identifier'] = xPlayer.identifier, ['@house'] = '{"owns":false,"furniture":[],"houseId":0}'})
            MySQL.Async.fetchAll("SELECT * FROM bought_houses", {}, function(result)
                TriggerClientEvent('loaf_housing:setHouse', xPlayer.source, json.decode('{"owns":false,"furniture":[],"houseId":0}'), result)
            end)
        end
    end)
end)
                ...
                TriggerEvent('esx_addoninventory:getInventory', 'housing', identifier, function(inventory)
                    if inventory.getItem(item)['count'] >= amount  then
                        if xPlayer.canCarryItem(item,amount) then
                            ...
                            TriggerClientEvent('esx:showNotification', src, "Nu ai destul loc in inventar")
                        end
                    else
...
ESX.RegisterServerCallback('loaf_housing:getPlayerWardrobe', function(source, cb)
	local xPlayer  = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('loaf_housing:removeOutfit')
