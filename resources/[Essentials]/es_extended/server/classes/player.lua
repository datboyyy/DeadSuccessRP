function CreateExtendedPlayer(player, accounts, inventory, job, loadout, name, lastPosition)
    ...
    self.query = MySQL.Sync.fetchAll("SELECT * FROM `users` WHERE `identifier` = @identifier", {['@identifier'] = self.player.getIdentifier()})
	self.crypto = 0

	if self.query[1] ~= nil then
		self.crypto = self.query[1].crypto

		self.getID = function()
			return self.query[1].id
		end
		...
		self.getPhoneNumber = function()
			return self.query[1].phone_number
		end

		self.getIBAN = function()
			return self.query[1].iban
		end

		self.getCrypto = function()
			return self.crypto
		end

		self.addCrypto = function(amount)
			self.crypto = self.crypto + tonumber(amount)
			MySQL.Async.execute('UPDATE users SET crypto = @crypto WHERE identifier = @identifier', {
		...
		self.removeCrypto = function(amount)
			self.crypto = self.crypto - tonumber(amount)
			MySQL.Async.execute('UPDATE users SET crypto = @crypto WHERE identifier = @identifier', {
				['@identifier'] = self.identifier,
				['@crypto'] = self.crypto
	...
	self.setAccountMoney = function(acc, money)
		...
		if acc == 'bank' then
			...
			TriggerClientEvent('st-banking:updateBank', self.source, newMoney)
		end
	...
	self.addAccountMoney = function(acc, money)
		...
		if acc == 'bank' then
			self.set('bank', newMoney)
			TriggerClientEvent('st-banking:addBank', self.source, money)
			TriggerClientEvent('st-banking:updateBank', self.source, newMoney)
		end
	...
	self.removeAccountMoney = function(acc, money)
		...
		if acc == 'bank' then
			self.set('bank', newMoney)
			TriggerClientEvent('st-banking:removeBank', self.source, money)
			TriggerClientEvent('st-banking:updateBank', self.source, newMoney)
		end
	...
	self.removeInventoryItem = function(name, count)
		...
		TriggerClientEvent('esx:removeInventoryAVACItem', self.source, item, count)
	end
	...
	self.setInventoryItem = function(name, count)
		...
		if oldCount > item.count  then
			...
			TriggerClientEvent('esx:removeInventoryAVACItem', self.source, item, oldCount - item.count)
		else
	...
	self.removeWeapon = function(weaponName, ammo)
		...
		if weaponLabel then
			...
			TriggerClientEvent('esx:removeInventoryAVACItem', self.source, {label = weaponLabel}, 1)
		end
	end
