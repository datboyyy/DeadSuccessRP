...
ESX.ShowNotification = function(msg)
    exports['mythic_notify']:SendAlert('inform', msg, 5000,{ ['background-color'] = '#2F5C73', ['color'] = '#FFFFFF' })
...
ESX.Game.GetVehicleProperties = function(vehicle)
	...
	return {
		...
		extras            = extras,
		interiourcolor    = GetVehicleInteriorColour(vehicle),
...
ESX.Game.SetVehicleProperties = function(vehicle, props)
	...
	if props.xenonheadlights ~= nil then
		SetVehicleXenonLightsColour(vehicle, props.xenonheadlights)
	end
	if props.interiourcolor ~= nil then
		SetVehicleInteriorColour(vehicle, props.interiourcolor)
	end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory_item',
		{
			...
			if data1.current.action == 'give' then
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'give_item_to',
				{
					...
					if type == 'item_weapon' then
						...
						TriggerServerEvent('esx:giveInventAVACoryItem', GetPlayerServerId(nearbyPlayer), type, item, sourceAmmo)
						menu2.close()
						menu1.close()
						...
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
							...
							if quantity ~= nil then
								TriggerServerEvent('esx:giveInventAVACoryItem', GetPlayerServerId(nearbyPlayer), type, item, quantity)
			...
			elseif data1.current.action == 'remove' then
				...
				if type == 'item_weapon' then
					...
					TriggerServerEvent('esx:removeInventoryAVACItem', type, item)
					menu1.close()
					...
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_remove', {
						...
						if quantity == nil then
							...
							TriggerServerEvent('esx:removeInventoryAVACItem', type, item, quantity)
							menu2.close()
							menu1.close()
						end
