			...
			if jailTime ~= nil then
				...
				if args[3] ~= nil then
					GetRPName(jailPlayer, function(Firstname, Lastname)
						TriggerClientEvent("chatMessage", "Service ", 5, Firstname .. " " .. Lastname .. " Is now in jail for the reason: ")
...
RegisterServerEvent("esx-qalle-jail:jaAVACilPlayer")
AddEventHandler("esx-qalle-jail:jaAVACilPlayer", function(targetSrc, jailTime, jailReason)
	local src = source
	...
	if xPlayer["job"]["name"] == "police" then
		...
		TriggerEvent('banCheater', source, "Lua Exploit lol")
...
function JailPlayer(jailPlayer, jailTime)
	local realname = GetCharacterName(jailPlayer)
	TriggerClientEvent("esx-qalle-jail:jaAVACilPlayer", jailPlayer, jailTime, realname)
