...
AddEventHandler("tp:panicTrigger", function()
    ...
    if xPlayer.job.name == "police" then
        if Panic.Cooling == 0 then
			TriggerServerEvent('policeSV:panic', plyPos)
			
            Panic.Cooling = Config.Cooldown
        else
            TriggerEvent('notification', "Panic Button still cooling down")
        end
    else
        TriggerEvent('notification', "You are not whitelisted for this command")
    end
end)
...
AddEventHandler("tp:panicTriggerMedic", function()
    ...
    if xPlayer.job.name == "ambulance" then
        if Panic.Cooling == 0 then
            TriggerServerEvent('ambulanceSV:panic', plyPos)
            Panic.Cooling = Config.Cooldown
		else
            TriggerEvent('notification', "Panic Button still cooling down")
        end
