...
Citizen.CreateThread( function()
	while true do 
		...
		while IsAbleToRob and not UnAuthJob() and (CopsOnline >= Config.MinPolice) and hasgun() do
			...
			plyloc = GetEntityCoords(ply)
			for i, v in pairs(Config.CaseLocations) do
				if GetDistanceBetweenCoords(plyloc, v.Pos.x, v.Pos.y, v.Pos.z, true) < 1.2  and not v.Broken and not IsBusy then
					sleep = 5
    ...
    while true do 
        ...
        if distance < 3.0 then
        	Wait(1)
             ...
             DT(-596.47, -283.96, 50.33, "[E] Use G6 Card")
             if IsControlJustReleased(0,38) and distance < 1.0 then
             	if exports["dsrp-inventory"]:hasEnoughOfItem("Gruppe6Card",1,false) then
					 ...
					 DoorSystemSetDoorState(48, false, false, true)
					 DoorSystemSetDoorState(49, false, false, true)
					 TriggerServerEvent('trp-doors:fetchState', 48, false)
					 TriggerServerEvent('trp-doors:fetchState', 49, false)
					 TriggerServerEvent('esx_JewelRobbery:policenotify')
             	end
             end
		else
