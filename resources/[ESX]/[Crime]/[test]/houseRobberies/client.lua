 ...
 if isNight() and not isRobbing then
  ...
  for id,v in pairs(robbableHouses) do
   if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 2.5 then
    ...
    exports["sway_taskbar"]:taskBar(12000,"Lockpicking ",false,false)
     ...
     TriggerEvent('esx-dispatch:houserobbery')
   ...
   for i=1,#myRobbableItems do
    if (GetDistanceBetweenCoords(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], GetEntityCoords(GetPlayerPed(-1))) < 1.4) and not myRobbableItems[i]['isSearched'] then
     ...
     if IsControlJustReleased(1, 74) then
      ...
      exports["sway_taskbar"]:taskBar(6500,"Searching",false,false)
...
AddEventHandler('SearchAnimation', function()
  ...
  while IsSearching do
   if not IsEntityPlayingAnim(PlayerPedId(), "mini@repair", "fixing_a_player", 3) then
    ...
    FreezeEntityPosition(PlayerPedId(), false)
    ClearPedTasks(PlayerPedId())
    TriggerServerEvent('dlrp_houseRobberies:searchItem')
   end
   Citizen.Wait(1)
  end
  ClearPedTasks(PlayerPedId())
...
RegisterNetEvent('houseRobberies:attemptPD')
AddEventHandler('houseRobberies:attemptPD', function(lockpicks)
 if isRobbing and DoesEntityExist(safe) then
  local playerCoords = GetEntityCoords(PlayerPedId(), true)
  if GetDistanceBetweenCoords(playerCoords, safepos.x, safepos.y, safepos.z, true) <= 3.0 then
  print('breach anim here')
  end
 end
 ...
 if isNight() and not isRobbing then
  ...
  for id,v in pairs(robbableHouses) do
   if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 2.5 then
    TaskPlayAnim(PlayerPedId(), "random@paparazzi@peek", "left_peek_a", 1.0, 1.0, 1.0, 1, 0.0, 0, 0, 0)
    exports["sway_taskbar"]:taskBar(12000,"Breaching Cribbo ",false,false)
