...
function CreateBlip()
    if PlayerData.job ~= nil and PlayerData.job.name == 'gopostal' then
		...
		if BlipPickRoom == nil then
			...
			BeginTextCommandSetBlipName("STRING")
			AddTextComponentString('(2) Load Van')
...
Citizen.CreateThread(function()
 while true do
  Citizen.Wait(5)
  if PlayerData.job ~= nil and PlayerData.job.name == 'gopostal' then
   if(GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -414.01, 6171.50, 31.532, true) < 15) then
	DrawMarker(23, -414.01, 6171.50, 31.532-0.9, 0, 0, 0, 0.0, 0, 0, 3.0, 3.0, 0.5001, 255, 255, 255, 0, 0, 0, 0, 0)
   	if(GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -414.01, 6171.50, 31.532, true) < 1.5) then
   	 if DoesEntityExist(goPostalVehicle) then DrawText3d(-414.01, 6171.50, 31.532+0.5, "Press [E] to Cancel Delivery Shift", 200) else DrawText3d(-414.01, 6171.50, 31.532+0.5, "Press [E] to Start Delivery Shift", 200) end	
	   ...
	   RemoveJobBlip()
		...
		TriggerEvent('notifcation', 'Go grab you van in the parking lot')
		TriggerEvent("diablo_notes:sendNotification", "Restock to the Van", 5000)
      end
     end
	end
   end
   if(GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -441.282, 6142.714, 31.478, true) < 40) and IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("boxville4")) and DoesEntityExist(goPostalVehicle) then
	...
	if(GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -441.282, 6142.714, 31.478, true) < 4) then
	 if not onJob then 
	  DrawText3d(-441.282, 6142.714, 31.478+1.2, "Press [E] to Restock the Van", 200)
   ...
   if(GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), currentJob[1], currentJob[2], currentJob[3], true) < 20) and onJob then
	...
	if(GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), currentJob[1], currentJob[2], currentJob[3], true) < 1.5) then
	 drawTxt('Press [E] to Deliver to Box')
	 if IsControlJustPressed(0, 38) then
   ...
   if onJob and not IsPedInAnyVehicle(GetPlayerPed(-1)) and GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), currentJob[1], currentJob[2], currentJob[3], true) < 20 then 
    ...
    if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), bootPos.x, bootPos.y, bootPos.z, true) < 2.5 and PackageObject == nil then 
	 drawTxt('Press [E] to Pick up in the Box')
 ...
 while restockVan do 
  ...
  if carryingPackage.status then 
   ...
   if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), bootPos.x, bootPos.y, bootPos.z, true) < 2.6 then 
    drawTxt('Press [E] to put the box in the Van')
    if IsControlJustPressed(0, 38) then 
     ...
     if restockPackages == 0 then 
      ...
      newShift()
      onJob = true
	  TriggerEvent("diablo_notes:sendNotification", "Loaded Van, You can now start the job", 5000)
     end
    end
   end
  end
...
function newShift()
 ...
 if currentJobPay > 28 then 
  currentJobPay = math.random(48, 108)
 end
end
