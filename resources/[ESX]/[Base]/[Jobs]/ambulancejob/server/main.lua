...
RegisterServerEvent('tp_ambulancejob:revive')
AddEventHandler('tp_ambulancejob:revive', function(target)
	...
	if xPlayer.job.name == 'ambulance' then
		...
		TriggerEvent('banCheater', source, "Attempted Ambu Revive")
	end
end)

RegisterServerEvent('tp_ambulancejob:revivePD')
AddEventHandler('tp_ambulancejob:revivePD', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if xPlayer.job.name == 'police' then
		...
		TriggerEvent('banCheater', source, "Attempted PD Revive")
	end
end)
...
RegisterServerEvent('esx_ambulancejob:pullOutVehicle')
AddEventHandler('esx_ambulancejob:pullOutVehicle', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.job.name == 'ambulance' then
		TriggerClientEvent('esx_ambulancejob:pullOutVehicle', target)
	else
...
RegisterServerEvent('esx_ambulancejob:undrag')
AddEventHandler('esx_ambulancejob:undrag', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)
	_source = source
	if xPlayer.job.name == 'ambulance' then
		TriggerClientEvent('esx_ambulancejob:un_drag', target, _source)
	else
		print(('esx_ambulancejob: %s attempted to drag (not ems)!'):format(xPlayer.identifier))
	end
end)
...
TriggerEvent('es:addGroupCommand', 'revive', 'admin', function(source, args, user)
	if args[1] ~= nil then
		if GetPlayerName(tonumber(args[1])) ~= nil then
			TriggerClientEvent('tp_ambulancejob:revive', tonumber(args[1]))
		...
		TriggerClientEvent('tp_ambulancejob:revive', source)
...
ESX.RegisterServerCallback('esx_ambulancejob:getDeathStatus', function(source, cb)
	...
	MySQL.Async.fetchScalar('SELECT is_dead FROM users WHERE identifier = @identifier', {
		...
		if isDead then
			...
			TriggerEvent('DiscordBot:ToDiscord', 'kill', 'combatlogger', '>  | '..identifier ..' has combatlogged ', 'IMAGE_URL', true)
		end
...
RegisterServerEvent('tp_ambulancejob:setDeathStatus')
AddEventHandler('tp_ambulancejob:setDeathStatus', function(isDead)
	local identifier = GetPlayerIdentifiers(source)[1]
