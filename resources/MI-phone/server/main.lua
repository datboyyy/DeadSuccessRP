...
function getOrGeneratePhoneNumber (sourcePlayer, identifier, cb)
    ...
    if myPhoneNumber == '0' or myPhoneNumber == nil then
        ...
        until id == nil
        MySQL.Async.insert("UPDATE users SET phone = @phone WHERE identifier = @identifier", {
            ['@phone'] = myPhoneNumber,
            ['@identifier'] = identifier
        }, function ()
            cb(myPhoneNumber)
...
function getIdentifierByPhoneNumber(phone) 
    local result = MySQL.Sync.fetchAll("SELECT users.identifier FROM users WHERE users.phone = @phone", {
        ['@phone'] = phone
    })
    if result[1] ~= nil then
...
function getNumberPhone(a)
    local b=MySQL.Sync.fetchScalar("SELECT users.phone FROM users WHERE users.identifier = @identifier",{["@identifier"]=a})
    if b~=nil then 
        return b 
    end;
    return nil 
        ...
        ExecuteSql(false, "SELECT * FROM player_contacts WHERE `identifier` = '"..Player.identifier.."' ORDER BY `name` ASC", function(result)
            ...
            ExecuteSql(false, "SELECT * FROM owned_vehicles WHERE `owner` = '"..Player.identifier.."'", function(garageresult)
                ...
                ExecuteSql(false, 'SELECT * FROM `player_mails` WHERE `identifier` = "'..Player.identifier..'" ORDER BY `date` ASC', function(mails)
                    ...
                    ExecuteSql(false, "SELECT * FROM `phone_messages2` WHERE `identifier` = '"..Player.identifier.."'", function(messages)
...
ESX.RegisterServerCallback('qb-crypto:server:GetCryptoTransactions', function(source, cb, ContactData)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    ExecuteSql(false, 'SELECT * FROM `crypto_transactions` WHERE `citizenid` = "'..xPlayer.identifier..'" ORDER BY `date` ASC', function(transactions)
            ...
            if TargetData ~= nil then
                ...
                ExecuteSql(false, "SELECT * FROM `phone_messages2` WHERE `identifier` = '"..SenderData.identifier.."' AND `number` = '"..ChatNumber.."'", function(Chat)
                    if Chat[1] ~= nil then
                        ...
                        -- Update for sender
                        ExecuteSql(false, "UPDATE `phone_messages2` SET `messages` = '"..json.encode(ChatMessages).."' WHERE `identifier` = '"..SenderData.identifier.."' AND `number` = '"..ChatNumber.."'")
                        ...
                        -- Insert for sender
                        ExecuteSql(false, "INSERT INTO `phone_messages2` (`identifier`, `number`, `messages`) VALUES ('"..SenderData.identifier.."', '"..ChatNumber.."', '"..json.encode(ChatMessages).."')")
                ...
                ExecuteSql(false, "SELECT * FROM `phone_messages2` WHERE `identifier` = '"..SenderData.identifier.."' AND `number` = '"..ChatNumber.."'", function(Chat)
                    if Chat[1] ~= nil then
                        ...
                        -- Insert for sender
                        ExecuteSql(false, "INSERT INTO `phone_messages2` (`identifier`, `number`, `messages`) VALUES ('"..SenderData.identifier.."', '"..Player[1].phone.."', '"..json.encode(ChatMessages).."')")
                    end
                end)
            end
        end
