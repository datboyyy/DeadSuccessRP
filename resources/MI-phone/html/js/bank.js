...
MI.Phone.Functions.DoBankOpen = function(data) {
    $(".bank-app-account-number").val("Account Number: " + data.iban);
    $(".bank-app-account-balance").html("$ "+data.bank.toFixed());
        ...
        if (amountData >= amount) {
            $.post('http://MI-phone/TransferMoney', JSON.stringify({
                ...
                if (data.CanTransfer) {
                    ...
                    MI.Phone.Notifications.Add("fas fa-university", MI.Phone.Functions.Lang("BANK_TITLE"), "You have transferred $ "+amount+"!", "#badc58", 1500);
                } else {
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('http://MI-phone/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                MI.Phone.Notifications.Add("fas fa-university", MI.Phone.Functions.Lang("BANK_TITLE"), "You paid an invoice of $"+InvoiceData.amount+"!", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
...
$(document).on('click', '.decline-invoice', function(event){
    ...
    MI.Phone.Notifications.Add("fas fa-university", MI.Phone.Functions.Lang("BANK_TITLE"), "You have paid $"+InvoiceData.amount+"!", "#badc58", 1500);
});
