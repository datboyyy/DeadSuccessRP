...
$(document).on('click', '.storeapp-remove', function(e){
    ...
    MI.Phone.Data.Applications[AppData.app] = null;

    $.post('http://MI-phone/RemoveApplication', JSON.stringify({
        app: AppData.app
    }));
    setTimeout(function(){
...
$(document).on('click', '.download-password-accept', function(e){
    ...
    if (FilledInPassword == TemplatePassword) {
        ...
        $(".download-progressbar-fill").animate({
            ...
            $.post('http://MI-phone/InstallApplication', JSON.stringify({
                app: CurrentApp,
            }), function(Installed){
                if (Installed) {
                    var applicationSlot = $(".phone-applications").find('[data-appslot="'+Installed.data.slot+'"]');
                    var blockedapp = IsAppJobBlocked(Installed.data.blockedjobs, MI.Phone.Data.PlayerJob.name)
                    if ((!Installed.data.job || Installed.data.job === MI.Phone.Data.PlayerJob.name) && !blockedapp) {
                    ...
                    MI.Phone.Data.Applications[Installed.data.app] = Installed.data;
