...
function SetupCryptoData(Crypto) {
    ...
    $(".crypto-action-page-wallet").html("Portfolio: "+CryptoData.Portfolio+" Qbit('s)");
    ...
    if ((Coins !== "") && (Price !== "")) {
        $.post('http://MI-phone/BuyCrypto', JSON.stringify({
            ...
            if (CryptoData !== false) {
                ...
                CloseCryptoPage()
                MI.Phone.Notifications.Add("fas fa-university", "bank", "$ "+ Price +" has been charged!", "#badc58", 2500);
            } else {
                MI.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "don't have enough money...", "#badc58", 1500);
            }
        });
    } else {
        MI.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Fill In Everything!", "#badc58", 1500);
    }
});
        ...
        if (CryptoData.Portfolio >= parseInt(Coins)) {
            $.post('http://MI-phone/SellCrypto', JSON.stringify({
                ...
                if (CryptoData !== false) {
                    UpdateCryptoData(CryptoData)
                    CloseCryptoPage()
                    MI.Phone.Notifications.Add("fas fa-university", "bank", "$ "+ Price +" has been credited", "#badc58", 2500);
                } else {
                    MI.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough Qbits..", "#badc58", 1500);
                }
            });
        } else {
            MI.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough Qbits..", "#badc58", 1500);
            ...
            if (WalletId !== CryptoData.WalletId) {
                $.post('http://MI-phone/TransferCrypto', JSON.stringify({
                    ...
                    if (CryptoData == "notenough") {
                        ...
                        MI.Phone.Notifications.Add("fas fa-university", "Crypto", "Wallet ID not in City!", "#badc58", 2500);
                    } else {
                        UpdateCryptoData(CryptoData)
                        CloseCryptoPage()
                        MI.Phone.Notifications.Add("fas fa-university", "Crypto", "You have transferred "+ Coins +", - to "+ WalletId +"!!", "#badc58", 2500);
                    }
                });
            } else {
                MI.Phone.Notifications.Add("fas fa-university", "Crypto", "You cannot transfer to yourself..", "#badc58", 2500);
            }
        } else {
            MI.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough Qbits..", "#badc58", 1500);
        }
    } else {
        MI.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Fill Everything In!", "#badc58", 1500);
    }
});
