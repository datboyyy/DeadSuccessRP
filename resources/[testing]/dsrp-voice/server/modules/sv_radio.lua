...
local function removePlayerFromRadio(sId, channelId)
    --TriggerClientEvent('lol:voice:radio:disconnect', source, channelId) --move to client
    ...
    if activeChannels[channelId].count == 0 then
        ...
        for k,v in pairs(activeChannels[channelId].subscribers) do
            TriggerClientEvent("lol:voice:radio:removed", k, channelId, sId)
            --TriggerClientEvent('lol:voice:targets:player:remove', k, sId, "radio", false)
        end
    end
...
local function addPlayerToRadio(sId, channelId, removeOld)
    ...
    for k,v in pairs(activeChannels[channelId].subscribers) do
        --TriggerClientEvent('lol:voice:targets:player:add', k, sId, "radio", false)
        TriggerClientEvent("lol:voice:radio:added", k, channelId, sId)
    end
    ...
    TriggerClientEvent('lol:voice:radio:connect', sId, channelId, activeChannels[channelId].subscribers)
    print("Adding player: " .. sId  .. " to channel: " .. channelId)
end


RegisterNetEvent("lol:voice:radio:addPlayerToRadio")
AddEventHandler("lol:voice:radio:addPlayerToRadio", function(channelId, removeOld)
    local sId = source
    addPlayerToRadio(sId, channelId, removeOld)
end)


RegisterNetEvent("lol:voice:radio:removePlayerFromRadio")
AddEventHandler("lol:voice:radio:removePlayerFromRadio", function(channelId)
    local sId = source
