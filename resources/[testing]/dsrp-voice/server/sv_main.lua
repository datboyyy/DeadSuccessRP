...
AddEventHandler("onResourceStart", function(resName) -- Initialises the script, sets up voice related convars
	...
	Debug("[lol:voice] Initialised Script -Swxy#0001")
end)


RegisterNetEvent("lol:voice:connection:state")
AddEventHandler("lol:voice:connection:state", function(state)
	TriggerClientEvent('lol:voice:connection:state', -1, source, state)
end)


RegisterNetEvent("lol:voice:transmission:state")
AddEventHandler("lol:voice:transmission:state", function(group, context, transmitting, isMult)
	--print('transmission')
	--print( type(group))
	...
	if isMult then
		for k,v in pairs(group) do
			-- print(v)
			-- print(k)
			TriggerClientEvent('lol:voice:transmission:state', v, _source, context, transmitting)
		end
	else
		TriggerClientEvent('lol:voice:transmission:state', group, _source, context, transmitting)
	end
	--else
	--end
end)
