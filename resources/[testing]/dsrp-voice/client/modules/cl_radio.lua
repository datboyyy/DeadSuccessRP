...
function StartTransmission()
    ...
    if not IsTalkingOnRadio then
        ...
        TriggerEvent('hud:talkonradio', true)
...
function StopTransmission(forced)
    ...
    RadioTimeout = TimeOut(300):next(function (continue)
        ...
        TriggerEvent('hud:talkonradio', false)
        Debug("[Radio] Transmission | Sending: %s | Radio ID: %s", IsTalkingOnRadio, CurrentChannel.id)
    end)
...
function LoadRadioModule()
    ...
    RegisterNetEvent("lol:voice:radio:connect")
    AddEventHandler("lol:voice:radio:connect", ConnectToRadio)

    RegisterNetEvent("lol:voice:radio:disconnect")
    AddEventHandler("lol:voice:radio:disconnect", DisconnectFromRadio)

    RegisterNetEvent("lol:voice:radio:added")
    AddEventHandler("lol:voice:radio:added", AddRadioSubscriber)

    RegisterNetEvent("lol:voice:radio:removed")
    AddEventHandler("lol:voice:radio:removed", RemoveRadioSubscriber)

    RegisterNetEvent("lol:voice:radio:power")
    AddEventHandler("lol:voice:radio:power", SetRadioPowerState)

    RegisterNetEvent("lol:voice:radio:volume")
    AddEventHandler("lol:voice:radio:volume", SetRadioVolume)
    ...
    TriggerEvent("lol:voice:radio:ready")
...
function PlayRemoteRadioClick(transmitting)
    if transmitting and Config.settings.remoteClickOn or not transmitting and Config.settings.remoteClickOff then
        ...
        if transmitting then
            TriggerEvent("InteractSound_CL:PlayOnOne","mic_click_on",0.1)
        else
            TriggerEvent("InteractSound_CL:PlayOnOne","mic_click_off",0.1)
        end
    end
end
...
function PlayLocalRadioClick(transmitting)
    if transmitting and Config.settings.localClickOn or not transmitting and Config.settings.localClickOff then
        ...
        if transmitting then
            TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 3.0, "mic_click_on", 0.1)
        else
            TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 3.0, "mic_click_off", 0.1)
        end
