    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if isVoiceActive and not VoiceEnabled then
                TriggerEvent('lol:voice:state', true)
            elseif not isVoiceActive and VoiceEnabled then
                TriggerEvent('lol:voice:state', false)
...
AddEventHandler('lol:voice:state', function(state)
    VoiceEnabled = state

    -- exports['tcm_hud']:toggleMumbleWarning(state)

    TriggerServerEvent("lol:voice:connection:state", state)
...
function AddPlayerToTargetList(serverID, context, transmit)
    if not Targets:targetContextExist(serverID, context) then
        ...
        if transmit then
            TriggerServerEvent("lol:voice:transmission:state", serverID, context, true, false)
        end
...
function RemovePlayerFromTargetList(serverID, context, transmit, refresh)
    if Targets:targetContextExist(serverID, context) then
        ...
        if transmit then
            TriggerServerEvent("lol:voice:transmission:state", serverID, context, false, false)
        end
...
function AddGroupToTargetList(group, context)
    ...
    TriggerServerEvent("lol:voice:transmission:state", group, context, true, true)
end
...
function RemoveGroupFromTargetList(group, context)
    ...
    TriggerServerEvent("lol:voice:transmission:state", group, context, false, true)
end
...
function SetVoiceProximity(proximity)
    ...
    TriggerEvent("hud:changeRange", proximity)
...
RegisterNetEvent("lol:voice:transmission:state")
AddEventHandler("lol:voice:transmission:state", function(serverID, context, transmitting)
...
RegisterNetEvent('lol:voice:targets:player:add')
AddEventHandler('lol:voice:targets:player:add', AddPlayerToTargetList)

RegisterNetEvent('lol:voice:targets:player:remove')
AddEventHandler('lol:voice:targets:player:remove', RemovePlayerFromTargetList)
