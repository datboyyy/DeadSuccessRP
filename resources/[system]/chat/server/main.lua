...
AddEventHandler('lol', function()
    local src = source
    local msg = rawCommand:sub(5)
    if player ~= false then
        local user = GetPlayerName(-1)
            TriggerClientEvent('chat:addMessages', -1, {
            template = '<div class="chat-message server"><b>Console:</b> Chat toggled.</div>',
            args = { msg }
        })
    end
end)

RegisterServerEvent('youshitlord')
AddEventHandler('youshitlord', function(bodyt,enginet,enginetempt,fueltankt,wheelt)
    local src = source
        local user = GetPlayerName(-1)
            TriggerClientEvent('chat:addMessages', -1, {
            template = '<div class="chat-message server"><b>System:<br></b> {0}<br>{1]<br>{2}<br>{3}<br>{4}<br>{5} </div>',
            args = { bodyt, enginet, enginetempt, fueltankt, wheelt}
        })
end)