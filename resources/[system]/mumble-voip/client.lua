    ...
    if channel <= mumbleConfig.radioClickMaxChannel then
        if mumbleConfig.micClicks then
            if (value and mumbleConfig.micClickOn) or (not value and mumbleConfig.micClickOff) then        
                local soundFile = 'mic_click_off'
                if value then  soundFile = 'mic_click_on' end
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustPressed(0, mumbleConfig.controls.proximity.key) then
			...
			if not (IsControlPressed(0, mumbleConfig.controls.radio.key)) and (mumbleConfig.controls.radio.pressed) then
				mumbleConfig.controls.radio.pressed = false
			end
...
exports("GetVoiceRange", VoiceRange)

function RadioStatus()
	return mumbleConfig.controls.radio.pressed
end
exports("GetRadioStatus", RadioStatus)
...
function CallStatus()
	local playerData = voiceData[playerServerId]
	return (playerData.call > 0)
end
exports("GetPhoneStatus", CallStatus)
