...
AddEventHandler('qb-lapraces:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    ...
    local esxChar = GetCharacterName(Player.identifier)
    ...
    if Races[RaceData.RaceId].Records ~= nil and next(Races[RaceData.RaceId].Records) ~= nil then
        if BLap < Races[RaceData.RaceId].Records.Time then
            ...
            TriggerClientEvent('MI-Phone:client:RaceNotify', src, 'Je hebt het WR van '..RaceData.RaceName..' verbroken met een tijd van: '..SecondsToClock(BLap)..'!')
        ...
        MySQL.Sync.execute("UPDATE `lapraces` SET `records` = '"..json.encode(Races[RaceData.RaceId].Records).."' WHERE `raceid` = '"..RaceData.RaceId.."'", {})
        TriggerClientEvent('MI-Phone:client:RaceNotify', src, 'Je hebt het WR van '..RaceData.RaceName..' neergezet met een tijd van: '..SecondsToClock(BLap)..'!')
    end
    ...
    if PlayersFinished == AmountOfRacers then
        if NotFinished ~= nil and next(NotFinished) ~= nil and NotFinished[RaceData.RaceId] ~= nil and next(NotFinished[RaceData.RaceId]) ~= nil then            for k, v in pairs(NotFinished[RaceData.RaceId]) do
                table.insert(LastRaces[RaceData.RaceId], {
    ...
    TriggerClientEvent('MI-Phone:client:UpdateLapraces', -1)
end)
...
AddEventHandler('qb-lapraces:server:JoinRace', function(RaceData)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    local srcident = GetPlayerIdentifier(src)
    local esxChar = GetCharacterName(Player.identifier)
    ...
    --TriggerClientEvent('MI-Phone:client:RaceNotify', creatorsource, esxChar.firstname..'. '..esxChar.lastname..' joined the race!')
    TriggerClientEvent('notification', src, 'You joined the race!', 1)
end)
...
AddEventHandler('qb-lapraces:server:LeaveRace', function(RaceData)
    ...
    local AvailableKey = GetOpenedRaceKey(RaceData.RaceId)
    print("cock-penis-1")
    print(AvailableRaces[AvailableKey].SetupSteam)
    local creatorsource = ESX.GetPlayerFromIdentifier(AvailableRaces[AvailableKey].SetupSteam).source
    if creatorsource ~= Player.source then
        TriggerClientEvent('notification', src, 'You left the race!', 1)
    end
    local AmountOfRacers = 0
    for k, v in pairs(Races[RaceData.RaceId].Racers) do
        AmountOfRacers = AmountOfRacers + 1
...
function GetCharacterName(identifier)
    ...
    return {firstname = "Unkown", lastname = "Unknown"}
end
