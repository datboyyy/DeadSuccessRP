...
RegisterCommand('testcom', function()
    TriggerEvent('qb-lapraces:client:StartRaceEditor', 'test')
end, false)
...
AddEventHandler('qb-lapraces:client:StartRaceEditor', function(RaceName)
    if not RaceData.InCreator then
        ...
        TriggerEvent('notification', 'Your already making a race', 1)
    end
end)
        ...
        while RaceData.InCreator do
            ...
            if PlayerVeh ~= 0 then
                ...
                if IsControlJustPressed(0, Keys["8"]) or IsDisabledControlJustPressed(0, Keys["8"]) then
                    if CreatorData.Checkpoints ~= nil and next(CreatorData.Checkpoints) ~= nil then
                        ...
                        TriggerEvent('notification', 'You have not placed a checkpoint ..', 1)
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["K"]) or IsDisabledControlJustPressed(0, Keys["K"]) then
                    if CreatorData.Checkpoints ~= nil and #CreatorData.Checkpoints >= 2 then
                        ...
                        TriggerEvent('notification', 'The minimum of checkpoints is 10', 1)
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["]"]) or IsDisabledControlJustPressed(0, Keys["]"]) then
                    if CreatorData.TireDistance + 1.0 ~= 16.0 then
                        ...
                        TriggerEvent('notification', 'You cant go higher than 15', 1)
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["["]) or IsDisabledControlJustPressed(0, Keys["["]) then
                    if CreatorData.TireDistance - 1.0 ~= 1.0 then
                        ...
                        TriggerEvent('notification', 'You cant go lower than 2', 1)
                    end
                end
            else
            ...
            if IsControlJustPressed(0, Keys["9"]) or IsDisabledControlJustPressed(0, Keys["9"]) then
                if not CreatorData.ConfirmDelete then
                    ...
                    TriggerEvent('notification', 'Press again on [9] to confirm', 1)
                else
                    ...
                    TriggerEvent('notification', 'Race-editor Canceled!', 1)
...
function SaveRace()
    ...
    TriggerEvent('notification', 'Race: ' .. CreatorData.RaceName .. ' is saved!', 1)
...
function DeleteCheckpoint()
    ...
    if RaceData.ClosestCheckpoint ~= 0 then
        if CreatorData.Checkpoints[RaceData.ClosestCheckpoint] ~= nil then
            ...
            TriggerEvent('notification', 'You cant go to fast.', 1)
        end
    else
        TriggerEvent('notification', 'You cant go to fast.', 1)
    end
...
AddEventHandler('qb-lapraces:client:RaceCountdown', function()
    ...
    if CurrentRaceData.RaceId ~= nil then
        while Countdown ~= 0 do
            if CurrentRaceData.RaceName ~= nil then
                if Countdown == 10 then
                    TriggerEvent('notification', 'The race will start in 10 seconds', 1)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                elseif Countdown <= 5 then
                    TriggerEvent('notification', Countdown, 1)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                end
        ...
        if CurrentRaceData.RaceName ~= nil then
            ...
            TriggerEvent('notification', 'GO!', 'success', 1)
        ...
        TriggerEvent('notification', 'You are currently notin a race..', 1)
    end
end)
...
function FinishRace()
    ...
    if CurrentRaceData.BestLap ~= 0 then
        TriggerEvent('notification', 'Race finished '..SecondsToClock(CurrentRaceData.TotalTime)..', Best Lap Time: '..SecondsToClock(CurrentRaceData.BestLap))
    else
        TriggerEvent('notification', 'Race finished ' ..SecondsToClock(CurrentRaceData.TotalTime), 1)
    end
    for k, v in pairs(CurrentRaceData.Checkpoints) do
        if CurrentRaceData.Checkpoints[k].blip ~= nil then
            RemoveBlip(CurrentRaceData.Checkpoints[k].blip)
...
AddEventHandler('qb-lapraces:client:PlayerFinishs', function(RaceId, Place, FinisherData)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            --TriggerEvent('Notification', FinisherData..' Finished on place : '..Place, 2)
            ESX.ShowNotification(FinisherData .. ' Finished on place : ' .. Place)
        end
    end
end)
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            TriggerEvent('notification', 'Go back to the start in 10 seconds: ' .. ToFarCountdown .. 's', 1)
                        else
