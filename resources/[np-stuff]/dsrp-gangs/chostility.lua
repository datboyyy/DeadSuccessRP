...
-- "AMBIENT_GANG_WEICHENG" -- little seoul {-759.36694335938,-927.24609375,18.555536270142},
 ...
 --   SetPedRelationshipGroupHash(GetPlayerPed(-1),GetHashKey('MISSION8'))
 --   SetPedRelationshipGroupDefaultHash(GetPlayerPed(-1),GetHashKey('MISSION8'))
...
Citizen.CreateThread(function()
  ...
  Wait(600)
    ...
    SetRelationshipBetweenGroups(1, GetHashKey("AMBIENT_GANG_LOST"), GetHashKey("AMBIENT_GANG_WEICHENG"))
    SetRelationshipBetweenGroups(1, GetHashKey("AMBIENT_GANG_WEICHENG"), GetHashKey("AMBIENT_GANG_LOST"))
    SetRelationshipBetweenGroups(1, GetHashKey("AMBIENT_GANG_LOST"), GetHashKey("AMBIENT_GANG_FAMILY"))
    SetRelationshipBetweenGroups(1, GetHashKey("AMBIENT_GANG_FAMILY"), GetHashKey("AMBIENT_GANG_LOST"))
    SetRelationshipBetweenGroups(1, GetHashKey("AMBIENT_GANG_LOST"), GetHashKey("AMBIENT_GANG_BALLAS"))
    SetRelationshipBetweenGroups(1, GetHashKey("AMBIENT_GANG_BALLAS"), GetHashKey("AMBIENT_GANG_LOST"))

    SetRelationshipBetweenGroups(1, GetHashKey("AMBIENT_GANG_LOST"), GetHashKey("AMBIENT_GANG_SALVA"))
    SetRelationshipBetweenGroups(1, GetHashKey("AMBIENT_GANG_SALVA"), GetHashKey("AMBIENT_GANG_LOST"))
    SetRelationshipBetweenGroups(1, GetHashKey("AMBIENT_GANG_LOST"), GetHashKey("AMBIENT_GANG_MEXICAN"))
    SetRelationshipBetweenGroups(1, GetHashKey("AMBIENT_GANG_MEXICAN"), GetHashKey("AMBIENT_GANG_LOST"))
...
AddEventHandler("gangs:setDefaultRelations",function() 
    ...
    SetRelationshipBetweenGroups(0, GetHashKey('PLAYER'), GetHashKey('MISSION8'))

    -- cops from scenarios love cops and ems logged in
    SetRelationshipBetweenGroups(0, GetHashKey('COP'), GetHashKey('MISSION2'))

    -- players love cops and ems
    SetRelationshipBetweenGroups(0, GetHashKey('PLAYER'), GetHashKey('MISSION2'))
...
AddEventHandler("gangs:setHatredFull",function()
    ...
    while timer > 0 do 
      timer = timer - 1
      curcoords = GetEntityCoords(GetPlayerPed(-1))
      if GetDistanceBetweenCoords(curcoords, startcoords, true) > 25.0 or IsPedFatallyInjured(GetPlayerPed(-1)) then
        timer = 0
      end
      Citizen.Wait(1)
    end
   -- TriggerEvent("ressurection:relationships:norevive") Disabled temp
    --TriggerEvent("gangs:setDefaultRelations") Disabled temp
end)
...
AddEventHandler("civilian:alertPolice",function(basedistance,alertType,objPassed,isGunshot,isSpeeder)
    ...
    if not daytime then
      ...
      basedistance = basedistance * 0.45
    end

    if alertType == "personRobbed" and not pd then
     -- TriggerServerEvent('phone:assistRequestSilent', 'AIcall26', 'A.I. Phone Call')
    ...
    local dst = 0
    if nearNPC then
        dst = GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),GetEntityCoords(nearNPC),true)
    end
    ...
    if alertType == "recklessdriver" and not pd then
     -- TriggerServerEvent('phone:assistRequestSilent', 'AIcall25', 'A.I. Phone Call')
      AlertReckless()
    end
    ...
    if alertType == "drugsale" and not underground and not pd then
      if dst > 12.0 and dst < 18.0 then
          DrugSale()
          --TriggerServerEvent('phone:assistRequestSilent', 'AIcall9', 'A.I. Phone Call')
      end
    end
    ...
    if alertType == "druguse" and not underground and not pd then
        if dst > 12.0 and dst < 18.0 then
          DrugUse()
          --TriggerServerEvent('phone:assistRequestSilent', 'AIcall8', 'A.I. Phone Call')
      end
    end
    ...
    if alertType == "carcrash" then
      ...
      CarCrash()
     -- TriggerServerEvent('phone:assistRequestSilent', 'AIcall7', 'A.I. Phone Call')
    end

    if alertType == "carcrashstuck" then
    ...
    if alertType == "death" and not underground then
      ...
      AlertDeath()
      --TriggerServerEvent('phone:assistRequestSilent', 'AIcall4', 'A.I. Phone Call')

      local roadtest2 = IsPointOnRoad(GetEntityCoords(GetPlayerPed(-1)), GetPlayerPed(-1))
    ...
    if alertType == "PDOF" and not robbing and not underground and not pd then
        ...
        if dst > 12.0 and dst < 18.0 then
            AlertPdof()
            --TriggerServerEvent('phone:assistRequestSilent', 'AIcall2', 'A.I. Phone Call')
        end
    end
    ...
    if alertType == "Suspicious" then
      ...
      AlertSuspicious()
      --TriggerServerEvent('phone:assistRequestSilent', 'AIcall10', 'A.I. Phone Call')
    end
    ...
    if alertType == "fight" and not underground and not pd then
      ...
      if armed then
       -- TriggerServerEvent('phone:assistRequestSilent', 'AIcall3a', 'A.I. Phone Call')
      else
       -- TriggerServerEvent('phone:assistRequestSilent', 'AIcall3', 'A.I. Phone Call')
      end
      
    end

    if alertType == "gunshot" and not pd then

      AlertGunShot()
      --TriggerServerEvent('phone:assistRequestSilent', 'AIcall6', 'A.I. Phone Call')
    end


    if alertType == "gunshotvehicle" and not pd then
    ...
    if alertType == "lockpick" and not pd then
        if dst > 12.0 and dst < 18.0 then
           ...
           -- TriggerServerEvent('phone:assistRequestSilent', 'AIcall5', 'A.I. Phone Call')
        end
    end
    ...
    if alertType == "robberyhouse" and not pd then
      AlertCheckRobbery2()
     -- TriggerServerEvent('phone:assistRequestSilent', 'AIcall24', 'A.I. Phone Call')
    end

    if alertType == "robbery" and not pd then
      AlertCheckRobbery()
     -- TriggerServerEvent('phone:assistRequestSilent', 'AIcall5', 'A.I. Phone Call')
    end
...
function checkedLocations()
  ...
  local aDist = GetDistanceBetweenCoords(coords["x"], coords["y"],coords["z"], 2100.650390625,2927.7692871094,-61.901878356934, true)
...
function CallChance()
  ...
  local coords = GetEntityCoords(GetPlayerPed(-1))
  local aDist = GetDistanceBetweenCoords(NoCalls[1]["x"], NoCalls[1]["y"],NoCalls[1]["z"], coords["x"],coords["y"],coords["z"], true)
  local bDist = GetDistanceBetweenCoords(NoCalls[2]["x"], NoCalls[2]["y"],NoCalls[2]["z"], coords["x"],coords["y"],coords["z"], true)
  local cDist = GetDistanceBetweenCoords(NoCalls[3]["x"], NoCalls[3]["y"],NoCalls[3]["z"], coords["x"],coords["y"],coords["z"], true)
  local dDist = GetDistanceBetweenCoords(lowCalls[1]["x"], lowCalls[1]["y"],lowCalls[1]["z"], coords["x"],coords["y"],coords["z"], true)
...
function AlertSuspicious()
    ...
    --TriggerServerEvent('AlertSuspicious', street1, sex, plyPos.x, plyPos.y, plyPos.z)
...
function DrugSale()
  ...
  --TriggerServerEvent('DrugSale', street1, sex, plyPos.x, plyPos.y, plyPos.z)
  ReportPreviousVehicle()
end
...
function DrugUse()
  ...
  --TriggerServerEvent('DrugUse', street1, sex, plyPos.x, plyPos.y, plyPos.z)
  ReportPreviousVehicle()
end

function CarCrashStuck()
  local street1 = GetStreetAndZone()
  local plyPos = GetEntityCoords(GetPlayerPed(-1),  true)
  if checkedLocations() then
    return
  ...
  --TriggerServerEvent('CarCrashStuck', street1, sex, plyPos.x, plyPos.y, plyPos.z)
end
...
function CarCrash()
  ...
  --TriggerServerEvent('CarCrash', street1, sex, plyPos.x, plyPos.y, plyPos.z)
  ReportPreviousVehicle()
end
...
function AlertDeath()
  ...
  --TriggerServerEvent('someoneDied', street1, sex, plyPos.x, plyPos.y, plyPos.z)
end
...
function AlertFight()
  ...
  if armed then
   -- TriggerServerEvent('meleeInProgressS2', street1, sex)
  else
...
function AlertpersonRobbed(vehicle)
  if DoesEntityExist(vehicle) then
    ...
    make = GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))
    ...
    local vehicleinformation = "Vehicle stolen from scene - MAKE: " .. make .. " | PLATE: " .. plate .. " | COLORS: " .. colors[color1] .. " on " .. colors[color2] 
   -- TriggerServerEvent("vehicleleavingscene", vehicleinformation)
  end
 ...
 -- TriggerServerEvent('robberyInProgress3', street1, sex)
  ReportPreviousVehicle()
end
...
function AlertCheckRobbery2()
 ...
 -- TriggerServerEvent('robberyInProgress4', street1, sex)
  ReportPreviousVehicle()
end

function AlertCheckRobbery()
  local street1 = GetStreetAndZone()
  local male = IsPedMale(GetPlayerPed(-1))
  local sex = "Male"
  if male then
  ...
  --TriggerServerEvent('robberyInProgress1', street1, sex)
  ReportPreviousVehicle()
end

function AlertGunShotCar()
  local street1 = GetStreetAndZone()
  local male = IsPedMale(GetPlayerPed(-1))
  local sex = "Male"
  if male then
...
function ReportPreviousVehicle()
  ...
  local vehicleinformation = "Vehicle leaving from scene - MAKE: " .. make .. " | PLATE: " .. plate .. " | COLORS: " .. colors[color1] .. " on " .. colors[color2] .. " heading " .. dir 
  --TriggerServerEvent("vehicleleavingscene", vehicleinformation)
...
function AlertGunShot()
 ...
 -- TriggerServerEvent("gunshotInProgress", street1, sex)
  ReportPreviousVehicle()
end
...
function AlertCheckLockpick(object)
    ...
    if not DoesEntityExist(targetVehicle) then
     -- TriggerServerEvent('thiefInProgressS3', street1, sex)
      return
    end
    ...
    --TriggerServerEvent('thiefInProgressS1', street1, sex, plate, make, color1, color2)
...
function AlertReckless()
 ...
 -- TriggerServerEvent('AlertReckless', street1, sex, plate, make, color1, color2, street2)
...
function canPedBeUsed(ped,isGunshot,isSpeeder)
    ...
    if GetDistanceBetweenCoords(curcoords, startcoords, true) < 10.0 then
      return false
    end    

    -- here we add areas that peds can not alert the police
    if GetDistanceBetweenCoords(curcoords, 1088.76, -3187.51, -38.99, true) < 15.0 then
      return false
    end  
...
AddEventHandler("TriggerAIRunning",function(p)
    ...
    while imdead == 1 or imcollapsed == 1 do
      Citizen.Wait(1)
      if not IsPedFacingPed(usingped, GetPlayerPed(-1), 15.0) then
          ClearPedTasksImmediately(ped)
          TaskLookAtEntity(usingped, GetPlayerPed(-1), 5500.0, 2048, 3)
          TaskTurnPedToFaceEntity(usingped, GetPlayerPed(-1), 5500)
          Citizen.Wait(3000)
