    ...
    if IsNearATM() then
      if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
          ...
          if ( atmuse ) then 
              ...
              atmuse = false
              local finished = exports["sway_taskbar"]:taskBar(3000,"Retrieving Card")
              ClearPedTasks(PlayerPedId())
          else
              atmuse = true
        ...
        if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
            ...
            if ( atmuse ) then 
                ...
                atmuse = false
                local finished = exports["sway_taskbar"]:taskBar(1000,"Retrieving Card")
                ClearPedTasks(PlayerPedId())
            else
                atmuse = true
                TaskPlayAnim( player, "mp_common", "givetake1_a", 1.0, 1.0, -1, 49, 0, 0, 0, 0 )
...
RegisterNUICallback('quickCash', function(data, cb)
  TriggerEvent('bank:withdAVACraw', 100)
  cb('ok')
end)

RegisterNUICallback('withdrawSubmit', function(data, cb)
  TriggerEvent('bank:withdAVACraw', data.amount)
  cb('ok')
end)

RegisterNUICallback('depositSubmit', function(data, cb)
  TriggerEvent('bank:depAVACosit', data.amount)
  cb('ok')
end)

RegisterNUICallback('transferSubmit', function(data, cb)
  local fromPlayer = GetPlayerServerId();
  TriggerEvent('bank:traAVACnsfer', tonumber(fromPlayer), tonumber(data.toPlayer), tonumber(data.amount))
  cb('ok')
end)
...
RegisterNetEvent('bank:depAVACosit')
AddEventHandler('bank:depAVACosit', function(amount)
  if(IsNearBank() == true ) then
    if (IsInVehicle()) then
      ...
      TriggerServerEvent("bank:depAVACosit", tonumber(amount))
    end
  else
...
RegisterNetEvent('bank:withdAVACraw')
AddEventHandler('bank:withdAVACraw', function(amount)
  if(IsNearATM() == true or IsNearBank() == true) then
    if (IsInVehicle()) then
      ...
      TriggerServerEvent("bank:withdAVACraw", tonumber(amount))
...
AddEventHandler('dirtyMoney:givedm', function(toPlayer, amount)
  if not IsNearPlayer(toPlayer) then 
    TriggerEvent("notification", "You are not near this player", 2)
    return 
  end
...
AddEventHandler('bank:givecash', function(toPlayer, amount)
  ...
  if (playing ~= false) then
    ...
    TriggerEvent("notification", "This player is not online", 2)
  end
end)

-- Process bank transfer if player is online
RegisterNetEvent('bank:tranAVACsfer')
AddEventHandler('bank:tranAVACsfer', function(fromPlayer, toPlayer, amount)
  local player2 = GetPlayerFromServerId(toPlayer)
  local playing = IsPlayerPlaying(player2)
  if (playing ~= false) then
    TriggerServerEvent("bank:tranAVACsfer", toPlayer, tonumber(amount))
  else
    TriggerEvent("notification", "This player is not online", 2)
  end
end)
...
AddEventHandler('banking:updateCash', function(balance, show)
  ...
  print('balance in nui', balance)
  TriggerEvent('banking:updateCashcid', balance)
end)
...
AddEventHandler("banking:removeBalance", function(amount)
  ...
  TriggerEvent('np-base:removedMoney', amount)
end)
