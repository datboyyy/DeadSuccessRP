...
AddEventHandler("rp:playerBecameJob", function(jobpassed, name, notify)
	job = jobpassed
    if not job then
        job = "None"
    end
...
AddEventHandler('np-items:SetAmmo', function(sentammoTable)
	if sentammoTable ~= nil then
		...
		GiveAmmoNow()
		Wait(300)
		print('ammo set', json.encode(sentammoTable))
	end
end)
...
RegisterCommand('ensureAmmo', function()
	local source = source
	TriggerServerEvent('np-weapons:set', source)
end, false)
	...
	while true do
		...
		if IsPedShooting(ped) then
			...
			if throwableWeapons[weapon] then
				if exports["dsrp-inventory"]:hasEnoughOfItem(weapon,1,false) then
		...
		if IsPedPlantingBomb(ped) then
			if exports["dsrp-inventory"]:hasEnoughOfItem("741814745",1,false) then
...
AddEventHandler('actionbar:ammo', function(hash,amount,addition)
	...
	if ammoTable == nil then
		TriggerServerEvent("np-weapons:set")
		Citizen.Wait(2000)
	end
...
AddEventHandler('armory:ammo', function()
	...
	TriggerEvent("actionbar:ammo",1627465347,150,true)
end)
...
function updateAmmoNow()
	...
	if type(ammoType) == 'number' then 
		...
		TriggerServerEvent("np-weapons:updateAmmo", json.encode(ammoTable))
...
function updateAmmo()
	...
	if type(ammoType) == 'number' then 
		...
		TriggerServerEvent("np-weapons:updateAmmo", json.encode(ammoTable))
		print(hash,newammo,ammoType,json.encode(ammoTable))
...
function AttachmentCheck(weaponhash)
	...
	if exports["dsrp-inventory"]:hasEnoughOfItem("silencer_l",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_AR_SUPP` )
	end

	if exports["dsrp-inventory"]:hasEnoughOfItem("silencer_l2",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_AR_SUPP_02` )
	end

	if exports["dsrp-inventory"]:hasEnoughOfItem("silencer_s",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_PI_SUPP` )
	end

	if exports["dsrp-inventory"]:hasEnoughOfItem("silencer_s2",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_PI_SUPP_02` )	
	end

	if exports["dsrp-inventory"]:hasEnoughOfItem("extended_ap",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_APPISTOL_CLIP_02` )	
	end

	if exports["dsrp-inventory"]:hasEnoughOfItem("extended_sns",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_SNSPISTOL_CLIP_02` )	
	end

	if exports["dsrp-inventory"]:hasEnoughOfItem("extended_micro",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_MICROSMG_CLIP_02` )	
	end

	if exports["dsrp-inventory"]:hasEnoughOfItem("MediumScope",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_SCOPE_MEDIUM` )	
	end

	if exports["dsrp-inventory"]:hasEnoughOfItem("SmallScope",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_SCOPE_SMALL` )	
	end


	if exports["dsrp-inventory"]:hasEnoughOfItem("TinyScope",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_SCOPE_MACRO` )	
	end

	if exports["dsrp-inventory"]:hasEnoughOfItem("extended_tec9",1,false) then
