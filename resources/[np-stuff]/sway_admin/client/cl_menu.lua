    ...
    Citizen.CreateThread(function()
        while WarMenu.IsMenuOpened("targetmenu") do
			...
			for i,thePlayer in ipairs(players) do
				if NetworkIsPlayerActive(thePlayer) then 
					if WarMenu.MenuButton("[" .. GetPlayerServerId(thePlayer) .. "] " .. GetPlayerName(thePlayer), "command") then self:DrawCommand(cmd) cb(thePlayer) NPX.Admin:GetCommandData(cmd).drawcommand() return end
            
				else
					if WarMenu.Button('['..GetPlayerServerId(thePlayer)..'] INSTANCED?',"command",true) then end 
				end 
			end
...
Citizen.CreateThread(function()
    ...
    local function DrawPlayers()       
            ...
            for i,thePlayer in ipairs(players) do
                if NetworkIsPlayerActive(thePlayer) then 
                    if WarMenu.Button('['..GetPlayerServerId(thePlayer)..'] '..GetPlayerName(thePlayer)) then end 
                else
                    if WarMenu.Button('['..GetPlayerServerId(thePlayer)..'] INSTANCED?',"",true) then end 
                end 
            end 
    ...
    local function DrawDev()
        ...
        if WarMenu.Button("Dev debug: " .. (NPX._Admin.Menu.DevDebug and "Disable" or "Enable")) and userRank == "dev" then 
            ...
            TriggerEvent("hud:enabledebug",NPX._Admin.Menu.DevDebug)
...
AddEventHandler("np-admin:openMenu", function()
        ESX.TriggerServerCallback('np-admin:checkperms', function(isAdmin)
            if isAdmin then
