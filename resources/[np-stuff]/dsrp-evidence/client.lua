...
AddEventHandler("rp:playerBecameJob", function(job, name, notify)
    ...
    if job ~= "ambulance" then isMedic = false end
    if job ~= "police" then isCop = false end
...
RegisterCommand('clear:area', function()
        ...
        while true do
            ...
            if GetHashKey("WEAPON_FLASHLIGHT") == GetSelectedPedWeapon(ped) or triggered then
                ...
                if IsControlJustReleased(0, 38) and minScan < 2.0 then
                    ...
                    local finished = exports["sway_taskbar"]:taskBar(3000, "Picking Up Item", "What?", true)
