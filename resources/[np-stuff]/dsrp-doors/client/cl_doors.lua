...
AddEventHandler("rp:playerBecameJob", function(job, name, notify)
    if isMedic and job ~= "ambulance" then isMedic = false isInService = false end
    if isCop and job ~= "police" then isCop = false isInService = false end
    if job == "police" then isCop = true isInService = true end
    if job == "ambulance" then isMedic = true isInService = true end
    print("my job:" .. job)
...
AddEventHandler( 'cell:doors', function(num)
    ...
    TriggerServerEvent("dsrp-doors:alterlockstate",tonumber(num))
end)
...
Citizen.CreateThread(function()  -- Calc door around player into an array , dist of 100 every 5 seconds
    ...
    plyCoords = GetEntityCoords(playerped)
    while true do
        for i = 1, #evo.DoorCoords do
            local distance = #(plyCoords - vector3(evo.DoorCoords[i]["x"], evo.DoorCoords[i]["y"], evo.DoorCoords[i]["z"]))
            if distance < distanceCheckStage1 then
                closeDoorsStage1[i] = evo.DoorCoords[i]
            else
    ...
    while true do
        ...
        if currentTargetDist < distvariable and targetDoor ~= nil then    
            ...
            if cZ(objCoords["z"],targetDoor["z"],limit) then
                ...
                if(evo.offSet[targetDoor["doorType"]]) then
                    if (type(targetDoor["doorType"]) == "number") then
                        local pos = evo.offSet[targetDoor["doorType"]]
                ...
                if  IsControlJustReleased(1,  Controlkey["generalUse"][1]) then
                    if OpenCheck(curClosestNum) then
                        ...
                        if shortcheck > 1.7 and isKeyDoor(curClosestNum) then
                            ...
                            TriggerServerEvent("dsrp-doors:alterlockstate",curClosestNum)
                        end
                    end
                end 
            end     
...
function OpenCheck(curClosestNum)
    ...
    local job = police --need to get the player job here
    ...
    for k,v in pairs(evo.rankCheck) do
...
function isKeyDoor(num)
    ...
    if evo.DoorCoords[num]["doorType"] == "prop_gate_prison_01" then
        return false
    end
    if evo.DoorCoords[num]["doorType"] == 1286392437 then
        return false
    end
    if evo.DoorCoords[num]["doorType"] == "v_ilev_fin_vaultdoor" then
        return false
    end
    if evo.DoorCoords[num]["doorType"] == -1603817716 then
        return false
    end
    if evo.DoorCoords[num]["doorType"] == 1501451068 then
        return false
    end
    if evo.DoorCoords[num]["doorType"] == -550347177 then
        return false
    end
