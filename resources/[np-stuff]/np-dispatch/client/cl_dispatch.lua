...
AddEventHandler('dispatch:toggleNotifications', function(state)
    ...
    if state == "on" then
        ...
        TriggerEvent('SendAlert', "Dispatch is now enabled.")
    elseif state == "off" then
        disableNotifications = true
        disableNotificationSounds = true
        TriggerEvent('SendAlert', "Dispatch is now disabled.")
    elseif state == "mute" then
        disableNotifications = false
        disableNotificationSounds = true
        TriggerEvent('SendAlert', "Dispatch is now muted.")
    else
        TriggerEvent('SendAlert', "You need to type in 'on', 'off' or 'mute'.")
    end
end)
...
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(0)
        if (IsControlJustReleased(0, 178) and (ESX.GetPlayerData().job.name == 'ambulance' or ESX.GetPlayerData().job.name == 'police') and not exports["ambulancejob"]:GetDeath()) then
...
function AlertFight()
  ...
  TriggerServerEvent('esx-outlawalertSV:combatInProgress', plyPos)
...
function AlertGunShot()
    Citizen.CreateThread(function() 
      ...
      TriggerServerEvent('esx-outlawalertSV:gunshotInProgress', plyPos)
...
function CarCrash()
  ...
  TriggerServerEvent('esx-alertsSV:vehiclecrash', plyPos)
...
function AlertCheckLockpick(object)
  ...
  if DoesEntityExist(targetVehicle) then
    ...
    TriggerServerEvent('esx-alertsSV:vehiclesteal', plyPos)
    end
end
...
function AlertCheckRobbery2()
  ...
  TriggerServerEvent('esx-alertsSV:houserobbery', plyPos)
  Wait(math.random(5000,15000))
...
function AlertJewelRob()
  ...
  TriggerServerEvent('esx-alertsSV:jewelrobbey', plyPos)
  Wait(math.random(5000,15000))
...
function AlertJailBreak()
  ...
  TriggerServerEvent('esx-alerts:jailbreak', plyPos)
  Wait(math.random(5000,15000))
...
function AlertDrugJob()
  ...
  TriggerServerEvent('drugs:OutlawBlipEvent', plyPos)
...
function AlertStoreRobbery()
  ...
  TriggerServerEvent('esx-alertsSV:storerobbery', plyPos)
...
AddEventHandler("esx-dispatch:carjacking",function(object)
  AlertCheckLockpick(object)
end)
...
function PrisonBreakRob()
  ...
  TriggerServerEvent('dispatch:svNotify', {
    dispatchCode = dispatchCode,
    firstStreet = 'Boiling Broke Penitentiary',
    esxder = esxder,
    eventId = eventId,
    isImportant = true,
    priority = 1,
    dispatchMessage = "Prison Break In Progress",
    origin = {
      x = 1850.46,
      y = 2586.77,
      z = 45.65
  ...
  TriggerServerEvent('esx-alertsSV:jailbreak', plyPos)
end