...
on('player:receiveItem', (id, amount,generateInformation, itemdata) => {
	if (personalWeight > 250) {
		emit("SendAlert","Failed to give " + id + " because you were overweight!",2)
		return
	}
...
function findSlot(ItemIdToCheck,amount, nonStacking) {
	...
	if (foundslot == 0) {
		emit("SendAlert","Failed to give " + ItemIdToCheck + " because you were full!",2)
	}
...
function OpenGui() {
	...
	//cash = exports.aquautility.getCash()
	emit("aqua-updatecash")
	...
	//}
	//setTimeout(()=>{
		//SendNuiMessage(JSON.stringify({response: "cashUpdate", weaponlicence: weaponsLicence,brought: brought,cop: cop}))
	//},250)
};
