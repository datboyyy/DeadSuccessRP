...
$(document).ready(function () {
  ...
  window.addEventListener('message', function (event) {
    ...
    if (item.response == "openGui") {
      ...
      if (itemList[item.id]) {
        $.post("http://dsrp-inventory/GiveItem", JSON.stringify([item.id, item.amount, item.generateInformation, true, itemList[item.id].nonStack, item.data, itemList[item.id].weight]));
      } else {
  ...
  for (i = 0; i < slotLimit + 1; i++) {
    ...
    if (isEmpty(inventory[i])) {
      ...
      if (itemList[itemid] !== undefined) {
        ...
        $.post("http://dsrp-inventory/updateMyQuality", JSON.stringify({
          item_id: itemid,
          slot: slot,
          quality: quality
        }));
