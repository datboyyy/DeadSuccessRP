...
function runRecycle()
	if not isDay() then
		TriggerEvent("SendAlert","You can only run the factory 7am-7pm!",2)
		return
	end
	isRunningCrate = true
	...
	print('success', randomchance)
	isRunningCrate = false
	else
	print('random', randomchance)
	TriggerEvent("SendAlert","Nice work, keep it up!")
    isRunningCrate = false
	end
end
...
listOn = false
	while true do
		...
		if GetDistanceBetweenCoords(PlayerPos, 994.38,-3099.98,-38.99, true) <= 2 then
		...
		if IsControlJustPressed(1, 38) and IsPedInAnyVehicle(GetPlayerPed(-1), false) ~= 1 then
			...
			if GetDistanceBetweenCoords(PlayerPos, 2221.7973632813,5577.0966796875,53.844783782959, true) <= 10 then		
				...
				if JustDriedWeed then
					TriggerEvent("SendAlert","You are too tired from drying weed, either wait 15 minutes or get help!")
				else
				...
				local finished = exports["sway_taskbar"]:taskBar(4000,"Picking Weed",false,false)
			...
			if GetDistanceBetweenCoords(PlayerPos, -1172.6, -1572.13, 4.67, true) <= 5 then						
				-- selling bud
				local smallbud = exports["dsrp-inventory"]:hasEnoughOfItem("smallbud",5,false)
				if smallbud then
			...
			if GetDistanceBetweenCoords(PlayerPos, 3803.58, 4444.46, 4.1, true) <= 15 then						
				...
				if JustPickedWeed then
					TriggerEvent("SendAlert","You are too tired from gathering weed, either wait 15 minutes or get help!")
				else
					if not JustDriedWeed then
						TriggerEvent("weed:spam-prevent","drying")
						JustDriedWeed = true
					end
					local wetbud = exports["dsrp-inventory"]:hasEnoughOfItem("wetbud",5,false)
					if wetbud then

						local finished = exports["sway_taskbar"]:taskBar(20000,"Drying Weed",false,false,playerVeh)
						if (finished == 100) then
			...
			if GetDistanceBetweenCoords(PlayerPos, 5.87, -1601.48, 29.3, true) <= 1 then						
				-- selling bud
				
				local smallbud = exports["dsrp-inventory"]:hasEnoughOfItem("freshmeat",5,false)
				if smallbud then
			...
			if GetDistanceBetweenCoords(PlayerPos, 994.38,-3099.98,-38.99, true) <= 1 and not isTrading then
