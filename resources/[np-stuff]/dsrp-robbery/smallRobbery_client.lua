...
RegisterNetEvent('rp-numbers:setjobs')
AddEventHandler('rp-numbers:setjobs', function(jobslist)
  JobCount = jobslist
...
RegisterCommand('bankez', function()
bankenabled = true
print(bankenabled)
end)
    ...
    if lockpick then
      ...
      -- if CopsOnline >= Config.PoliceRequired then
        ...
        if VaultDoor ~= 0 then
          ...
          local finished = exports["sway_taskbar"]:taskBar(1000,"Requesting Access")
          ...
          local finished = exports["sway_taskbar"]:taskBar(35000,"Requesting Access")
          if finished ~= 100 or not passCard then
              TriggerEvent("notification","Incorrect card or you cancelled..",2)
              return
          end
...
AddEventHandler("robbery:secure", function()
  ...
  local finished = exports["sway_taskbar"]:taskBar(25000,"Securing Bank")
...
AddEventHandler("robbery:giveleitem", function(confirmed,slotid)
    ...
    if confirmed then
      if slotid < 5 then
        -- low quality
        TriggerServerEvent( 'mission:complAVACeted', 1000)
...
function GiveRareItem()
  ...
  if math.random(100) > 65 then
    ...
    elseif roll == 4 then
      ...
      TriggerServerEvent( 'mission:complAVACeted', math.random(1500,3500) )
    end

  else
    TriggerServerEvent( 'mission:complAVACeted', math.random(500,2500) )
  end
...
function DrawTextAndScan(x,y,z, inputType)
    ...
    if dst < 1.0 then
      if IsControlJustPressed(0,38) then
        ...
        local finished = exports["sway_taskbar"]:taskBar(timer,"Opening")
        if finished == 100 then
