...
Citizen.CreateThread(function()
    Citizen.Wait(1)
	while true do
	local x, y, z = 1208.7310791016,-3115.1735839844,5.5403270721436
    local ped = GetPlayerPed(-1)
	local pos = GetEntityCoords(ped)
	local texttodraw = "Press E to decrypt"
    local distance = GetDistanceBetweenCoords(pos.x,pos.y,pos.z,x, y, z,false)
        if distance <= 1.5 then
			...
			if IsControlJustReleased(0, 86) then
				...
				if exports["dsrp-inventory"]:hasEnoughOfItem("decrypterenzo",1,false) then			
					TriggerEvent("Animation:typeforrob")
					local finished = exports["sway_taskbar"]:taskBar(40000,"Decrypting Data")
					if finished == 100 then					
						FreezeEntityPosition(PlayerPedId(),false)						
						TriggerEvent('lucky:store')    
						TriggerEvent("inventory:removeItem", "decrypterenzo", 1)
						TriggerEvent("notification", "You have received a email!")
					end
				else
					FreezeEntityPosition(PlayerPedId(),false)	
					TriggerEvent("notification", "Your missing something")
				end
...
AddEventHandler("client:newStress", function()
	TriggerEvent('esx_status:add','stress',110000)
	end)
...
AddEventHandler("police:notifySecurityCam", function(currentRobbery)
    ...
    if jobname == "police" then
		TriggerEvent("notification", "Security Camera number " .. currentRobbery .. " has been triggered.", 1)
	end
end)
...
AddEventHandler("robbery:safe", function()
	...
	TriggerEvent("player:receiveItem","band",math.random(10))
