...
AddEventHandler("robbery:sendMarkers", function(markerSent,particlePosSent,markerDifficultSent)
    markers = markerSent
    particlePos = particlePosSent
    markerDifficult = markerDifficultSent
end)
        ...
        if markers ~= nil then
            ...
            for k,j in pairs(compiledMarkers) do
                ...
                if isCop or (not flags[k].isFinished and not continue) then
                    ...
                    if dist < 5 and isCop then DrawMarker(27,v["pos"][1],v["pos"][2],v["pos"][3], 0, 0, 0, 0, 0, 0, 0.69, 0.69, 0.3, 100, 255, 255, 60, 0, 0, 2, 0, 0, 0, 0) end
                    if dist < 5 and not isCop then 
                        local rgb = {250,255,190}
                        if haveNotcompleted then
                            rgb = {255,0,0}
                        end
                        DrawMarker(27,v["pos"][1],v["pos"][2],v["pos"][3], 0, 0, 0, 0, 0, 0, 0.60, 0.60, 0.3, rgb[1], rgb[2], rgb[3], 60, 0, 0, 2, 0, 0, 0, 0) 
                    end
                ...
                if not isCop and flags[k].isFinished and dist < 30 then
                    isNear = true
                    DrawMarker(27,v["pos"][1],v["pos"][2],v["pos"][3], 0, 0, 0, 0, 0, 0, 0.60, 0.60, 0.3, 222, 2, 2, 60, 0, 0, 2, 0, 0, 0, 0) 
                end
...
local activePolice = 5
    ...
    if activePolice < 0 then
        TriggerEvent("notification","Need more Police",2) 
        return
    end
    ...
    if (marker["group"] == "mainBank" or marker["group"] == "prisonGates") and (states["Prison_Power"]) then
        TriggerEvent("notification","The vault is still powered..",2) 
        return
    end

    if locationID == 36 then 
       if not states["Prison_Electric"] or not states["Prison_Physical"] then
            TriggerEvent("notification","You must not leave others behind , finish the other locks before this one.",2) return
        end 
    end

    if flags[locationID].inUse then TriggerEvent("notification","Someone Is Already Using this.",2) return end
    if flags[locationID].isFinished then TriggerEvent("notification","This Lock has already been broken.",2) return end

    local card = 1
    if marker["group"] == "mainBank" then card = CURRENT_CARD_CITY elseif marker["group"] == "paletoBank" then card = CURRENT_CARD_PALETO end


    if marker.toolType == ELECTRICAL_LOCK and not (itemID == "electronickit" or itemID == "thermite") then TriggerEvent("notification","Not the right tool for this job.",2) return end
    if marker.toolType == PHYSICAL_LOCK_PICK and not (itemID == "lockpick" or itemID == "thermite") then TriggerEvent("notification","Not the right tool for this job.",2) return end
    if marker.toolType == PHYSICAL_LOCK_THERMITE and itemID ~= "thermite" then TriggerEvent("notification","Not the right tool for this job.",2) return end

    if marker.toolType == AIR_LOCK and itemID ~= "locksystem" then TriggerEvent("notification","Not the right tool for this job.",2) return end

    if marker.toolType == CARDED_LOCK then
        if itemID == "electronickit" then
            TriggerEvent("notification","Must be started with a card , both will be used for this lock.",2)
            return
        elseif itemID ~= "Gruppe6Card2" then
            TriggerEvent("notification","Not the right tool for this job.",2)
            return
        end
    end
    ...
    if itemID == "lockpick" then
        ...
        local finished = exports["dsrp-lockpick"]:lockpick(pick[1],pick[2],pick[3],pick[4])
        ClearPedTasks(PlayerPedId())
        if finished == 100 then 
            TriggerServerEvent("robbery:robberyFinsihed",locationID,marker.toolType,itemNum)
        else
    ...
    if itemID == "locksystem" then
        TriggerServerEvent("robbery:robberyFinsihed",locationID,marker.toolType,itemID)
    ...
    if itemID == "Gruppe6Card2" then
        ...
        if exports["dsrp-inventory"]:hasEnoughOfItem("electronickit",1,false) then
            local card = exports["sway_taskbar"]:taskBar(9000,"Inserting Card")
            if card == 100 then
                Wait(400)
                TriggerEvent("animation:cancel")
                TriggerServerEvent("robbery:alarmTrigger",locationID)
                local rdif = exports["sway_taskbar"]:taskBar(6000,"Hacking..")
                if  rdif == 100 then
                    local kit = difficult.kit
            ...
            TriggerEvent("notification","Both a card and hacking device needed for this lock.",2)
        end
...
function thermiteHandle(locationID,itemID)
    ...
    local taskFinished = exports["sway_taskbar"]:taskBar(1000,"Getting Thermite ready")
...
function mycb(success,locationID,timeremaining)
    ...
    if success then
        TriggerServerEvent("robbery:robberyFinsihed",locationID,markers[locationID].toolType,43)
