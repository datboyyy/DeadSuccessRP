...
function pinFailure() {
  $.post('http://dsrp-stash/failure', JSON.stringify({pinResult: false}));
  gameOver = true;
  $('#modal').fadeIn();
  ++failedAttempts;
  if (failedAttempts >= 3)
...
function pinSuccess(pElevated) {
  $.post('http://dsrp-stash/complete', JSON.stringify({pinResult: true, owner: pElevated}));
...
document.onkeyup = function (data) {
  if (data.which == 27 ) {
    $.post('http://dsrp-stash/close', JSON.stringify({pinResult: false}));
  }
};
