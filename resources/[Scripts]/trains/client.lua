...
AddEventHandler("AskForTrainConfirmed", function()
	...
	if metroDist < countryDist then
		if metroDist < 25.0 then 
			local finished = exports["sway_taskbar"]:taskBar(28000,"Waiting for Train",false,true)	
		...
		if countryDist < 25.0 then 
			...
			if finished == 100 then			
				...
				TriggerEvent('chat:addMessage', -1, {
					template = '<div style="padding: 0.475vw; padding-left: 0.8vw; padding-right: 0.7vw; margin: 0.1vw; background-color: rgba(63, 126, 181, 0.9); border-radius: 7.5px;"><span style="font-weight: bold;">THOMAS: Choo Choo..{0}:</span> {1}</div>',
					args = {}
				})
			end
		else
...
function StartMetro(coords)
	...
	MetroTrain = CreateMissionTrain(24,coords[1],coords[2],coords[3],true)
	...
	TriggerEvent('chat:addMessage', {
		template = '<div style="padding: 0.6vw; padding-left: 0.8vw; background-color: rgba(67, 157, 217, 0.9); border-radius: 6px;"><span style="width: 100%; font-weight: bold;">Train: </span>Train will depart in 30 seconds.</div>',
		args = {}
	}) 
	Citizen.Wait(60000)
