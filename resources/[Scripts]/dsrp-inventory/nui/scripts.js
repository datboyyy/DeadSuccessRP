...
$(document).ready(function () {
  ...
  window.addEventListener('message', function (event) {
    ...
    if (item.response == "openGui") {
      ...
      if (itemList[item.id]) {
        $.post("http://dsrp-inventory/GiveItem", JSON.stringify([item.id, item.amount, item.generateInformation, true, itemList[item.id].nonStack, item.data]));
      } else {
        $.post("http://dsrp-inventory/GiveItem", JSON.stringify([item.id, item.amount, item.generateInformation, false, itemList[item.id].nonStack, item.data]));
      }
    }
  })
...
function invStack(targetSlot, moveAmount, targetInventory, originSlot, originInventory, purchase, itemCosts, itemidsent, amountmoving, crafting, weapon, amountRemaining) {
  ...
  $.post("http://dsrp-inventory/stack", JSON.stringify(arr));
};
...
function invMove(targetSlot, originSlot, targetInventory, originInventory, purchase, itemCosts, itemidsent, amountmoving, crafting, weapon) {
  ...
  $.post("http://dsrp-inventory/move", JSON.stringify(arr));
};
...
function invSwap(targetSlot, targetInventory, originSlot, originInventory) {
  ...
  $.post("http://dsrp-inventory/swap", JSON.stringify(arr));
};

function removeCraftItems(itemid, moveAmount) {
  let arr = itemList[itemid].craft
  let amount = moveAmount;
  $.post("http://dsrp-inventory/removeCraftItems", JSON.stringify([arr, amount]));
};
...
function DisplayInventoryMultiple(playerinventory, itemCount, invName, targetinventory, targetitemCount, targetinvName, cash, Owner) {
  ...
  if (targetinvName.indexOf("Drop") > -1) {
    ...
    slotLimitTarget = 70;
  } else if (targetinvName.indexOf("motel-bed") > -1) {
    secondaryMaxWeight = 600.0
    slotLimitTarget = 30;
  } else if (targetinvName.indexOf("motel-stash") > -1) {
    secondaryMaxWeight = 600.0
    slotLimitTarget = 50;
...
function BuildDrop(brokenSlots) {
  $.post("http://dsrp-inventory/dropIncorrectItems", JSON.stringify({
    slots: brokenSlots
  }));
}
...
function UpdateSetWeights() {
  ...
  $.post("http://dsrp-inventory/Weight", JSON.stringify({
    weight: meh
  }));
}
...
function DragToggle(slot,using) {
  ...
  if (slot) {
    ...
    if (occupiedslot == true && dragging == false) {
      ...
      $.post("http://dsrp-inventory/SlotInuse", JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function DropItem(slot, amountDropped) {
  ...
  // $.post('http://dsrp-inventory/dropitem', JSON.stringify({
...
function ErrorMove() {
  // $.post('http://dsrp-inventory/move:fail', JSON.stringify({}));
}

function SuccessMove() {
  // $.post('http://dsrp-inventory/move:success', JSON.stringify({}));
}
...
function CompileStacks(targetSlot, originSlot, inv1, inv2, originAmount, remainingAmount, targetAmount, purchase, itemCosts, itemidsent, moveAmount) {
  ...
  $.post("http://dsrp-inventory/SlotJustUsed", JSON.stringify({
...
function SwapStacks(targetSlot, originSlot, inv1, inv2) {
  // $.post('http://dsrp-inventory/swapstack', JSON.stringify({
  //   slot1: slot1,
  //   slot2: slot2,
  //   inv1: inv1,
  //   inv2: inv2,
...
function closeInv(pIsItemUsed = false) {
  ...
  $.post("http://dsrp-inventory/ServerCloseInventory", JSON.stringify({
    name: TargetInventoryName
  }));
  TargetInventoryName = "none";
  $.post("http://dsrp-inventory/Close", JSON.stringify({
...
function useitem() {
  if (dragging != false) {
    ...
    if (inventoryUsedName == PlayerInventoryName) {
      ...
      $.post("http://dsrp-inventory/invuse", JSON.stringify(arr));
