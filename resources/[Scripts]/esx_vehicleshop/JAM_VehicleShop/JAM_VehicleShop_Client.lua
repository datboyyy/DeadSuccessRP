				  ...
				  self.CurBuying = true
									if valid then 
										...
										ESX.Game.SpawnVehicle(nearestModel,spawnPos.xyz, spawnPos.w, function(cbVeh)
											...
											TriggerServerEvent("garage:addKeys", cbVeh, plate)
											TriggerServerEvent('sendvehtoserver', vehName, vehPrice, plate)
									        Citizen.Wait(10)
									        TaskWarpPedIntoVehicle(GetPlayerPed(-1), cbVeh, -1)
											local vehProps = ESX.Game.GetVehicleProperties(cbVeh)
											local vehiclemodelname = GetDisplayNameFromVehicleModel(cbVeh)
											TriggerServerEvent('JAM_VehicleShop:CompletePurchase', vehProps, vehName)
