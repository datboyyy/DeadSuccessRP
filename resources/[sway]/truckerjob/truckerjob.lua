...
function DepositTruckDocks()
  local vehFront = VehicleInFront()
    if vehFront > 0 then
      print(GetEntityModel(vehFront),GetHashKey(trailer))

      if GetEntityModel(vehFront) == GetHashKey(trailer) then
        local finished = exports["sway_taskbar"]:taskBar(25000,"Storing Trailer Contents")
        if finished == 100 then
            TriggerServerEvent("inv:delete","Trunk-"..GetVehicleNumberPlateText(vehFront))
        end
      else
        TriggerEvent("SendAlert", "This trailer appears to be the wrong one..",2)
      end

  else
      TriggerEvent("SendAlert", "Cant find vehicle..",2)
  end  
end
...
Citizen.CreateThread(function()
  while true do
    ...
    Wait(1)
    if ESX.PlayerData.job == "sway" and existingVeh ~= nil then
      if job ~= 0 and stage == "pickup" then
        if currentJobs[currentJobPos] == nil then
          TriggerEvent("SendAlert","Looks like the job has been taken,find a new job in the phone.",1)
          clearBlip()
          stage = "none"
          job = 0
      ...
      elseif job ~= 0 and stage == "drop" then
        ...
        elseif currentJobs[currentJobPos].JobType ~= "Shops" then
          local dist = #(vector3(currentJobs[currentJobPos].drop[1], currentJobs[currentJobPos].drop[2], currentJobs[currentJobPos].drop[3]) - GetEntityCoords(PlayerPedId()))
          if dist < 20 then
            TriggerEvent("SendAlert","You have arrived at drop point , start unpacking good's.",1)
            clearBlip()
            runDrop()
            stage = "none"
      ...
      elseif job ~= 0 and stage == "waiting" then
        if waitingForConfirmation then
          TriggerEvent("SendAlert","Delivery Accepted , Start unpacking",1)
          clearBlip()
          runDrop()
          stage = "none"
          TriggerServerEvent("trucker:jobFinished",job)
...
Citizen.CreateThread(function()
  while true do
    Wait(9000)
    if job ~= 0 and ESX.PlayerData.job == "sway"  then
      currentJobPos = returnJobPos(job)
      checkDestruction()
    end
  end
    ...
    if currentJobs[currentJobPos] ~= nil and currentJobs[currentJobPos].JobType == "Shops" then
      ...
      if dist < 20 then
        ...
        if(distance ~= -1 and distance < 5) then
            TriggerEvent("SendAlert","Requesting Acceptance from person.",1)
        ...
        TriggerEvent("SendAlert","You have to be near the drop location.",1)
      end
    else
      TriggerEvent("SendAlert","No valid job found.",1)
    end
  else
    TriggerEvent("SendAlert","No valid job found.",1)
  end
...
AddEventHandler("Trucker:SelectJob", function(data)
    ...
    stage = "pickup"
    notspawned = true
    TriggerEvent("SendAlert","Job Selected!",1)
end)
...
function runPickup()
    TriggerEvent("SendAlert","Pickup Started!",1)
...
function runDrop()
  TriggerEvent("SendAlert","Drop Started!",1)
    local dropOffSet = {}
...
function spawnTruck(i)
      ...
      TriggerEvent("SendAlert","Use Phone to check job",1)
        ...
        if #(vector3(v[1],v[2],v[3]) - GetEntityCoords(PlayerPedId())) < 5.5 then
          ...
          if(IsControlJustPressed(1, Controlkey["generalUse"][1])) then
            ...
            if job ~= 0 then
              ...
              TriggerEvent("SendAlert","You need a Job Set - Sign on as trucker then Open your phone to select a available delivery.",2)
            end
