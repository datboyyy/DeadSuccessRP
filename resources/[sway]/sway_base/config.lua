Config = {}

Config.Map = {
  {name="Benny's Original Motor Works",color=7, id=446,x = -205.28, y = -1304.52, z = 1},
  {name="Court House",color=0, id=419,x = 244.48, y = -387.04, z = 1},
  {name="GoPostal",color=5, id=541,x = 70.98, y = 114.30, z = 1},
  {name="Police Department",color= 68, id=60,x =  423.04, y = -980.08, z = 1},
  {name="Job Center", colour=17, id=351, x= 172.78, y= -26.89, z= 1},
  {name="Discount Furniture Store", colour=78, id=177, x= 803.08, y= -991.2, z= 1},
  {name="Pillbox Medical Center  ", colour=2, id=61, x= 290.52, y=-586.28, z= 1},
  {name="Movie Star Way [Studio]",color=11, id=136,x =  -926.68, y = -460.0, z = 1},
 -- {name="AutoCare  ",color=26, id=446,x = 946.0, y = -986.32, z = 1},
  {name="SplitSides Comedy Club",color=1, id=102,x = -432.84, y = 258.52, z = 1},
 --{name="[MC] Mechanic ",color=0, id=446,x = 982.8, y = -135.04, z = 1},
 -- {name="Cluckin Bell ",color=4, id=52,x = 177.12, y = -1044.68, z = 1},
  {name="Taco Shop ",color=4, id=52,x = 10.84, y = -1607.16, z = 1},
  {name="Recycling plant ",color=17, id=304,x = 746.75518798828, y=-1400.094482421, z=1},
  

}