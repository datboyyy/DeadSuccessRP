...
local bennyLocationTuner = vector3(982.8, -135.04, 74.08)
local bennyLocationImport = vector3(938.15,-970.96, 39.54)
...
AddEventHandler('event:control:bennys', function(useID)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        ...
        elseif useID == 3 and not isPlyInBennys then
...
Citizen.CreateThread(function()
    while true do 
        ...
        if IsPedInAnyVehicle(plyPed, false) then
            ...
            if nearDefault or nearImport or nearTuner then
					...
					if IsControlJustReleased(0, 38) then
                      if ESX.PlayerData.job.name == 'mechanic' then					  
						enterLocation(bennyLocation)
						 ...
						 ESX.ShowNotification("You're not mechanic")
					   end
					end	
                ...
                if isNear(plyPos, bennyLocationTuner, 2) then
                    if not isPlyInBennys then
                        Draw3DText(bennyLocationTuner.x, bennyLocationTuner.y, bennyLocationTuner.z + 0.5, "[Press ~p~E~w~ - Enter Biker Shop]", 255, 255, 255, 255, 4, 0.45, true, true, true, true, 0, 0, 0, 0, 55)
                        if IsControlJustReleased(0, 38) then
                            if ESX.PlayerData.job.name == 'biker' then					  
                              enterLocation(bennyLocationTuner)
                              else
                               ESX.ShowNotification("You're not mechanic")
                             end
