...
anims = {
    ...
    ["umbrella"] = function(ped)
        if obj ~= 0 then
            ...
            TriggerEvent("destroyPropPerm")
            if exports["dsrp-inventory"]:hasEnoughOfItem("umbrella", 1) then

                TriggerEvent("actionbar:setEmptyHanded")

                local finished = exports["sway_taskbar"]:taskBar(2500,"Opening Umbrella")
    ...
    ["sitchair"] = function(ped)
        ...
        if IsEntityPlayingAnim(ped, animDict, animation, 3) then
            ...
            loadAnimDict(animDict)
            local animLength = GetAnimDuration(animDict, animation)
            TaskPlayAnim(PlayerPedId(), animDict, animation,  1.0, 1.0, -1, 9, -1, 0, 0, 0)
        end
    end,
    ...
    ["smoke"] = function(ped)
        if exports["dsrp-inventory"]:hasEnoughOfItem("ciggy", 1) then
            ClearPedTasks(PlayerPedId())
    ...
    ["cigar"] = function(ped)
        if exports["dsrp-inventory"]:hasEnoughOfItem("cigar", 1) then
            local animDict = "amb@world_human_smoking@male@male_a@enter"
            local animation = "enter"
            if IsPedArmed(ped, 7) then
                SetCurrentPedWeapon(ped, 0xA2719263, true)
...
function StressTest(animName)
    if (animName == "pushups" or animName == "situps" or animName == "chinups" or
        ...
        (#(GetEntityCoords(PlayerPedId()) - vector3(1744.53, 2646.94, 45.61)) <
            ...
            10.0) then
        local finished =
            exports["sway_taskbar"]:taskBar(15000, "Relieving Stress")
        if finished == 100 then
...
RegisterCommand('em', function(source, args, raw) TriggerEvent('emotes:OpenMenu', args[1]) end)
...
RegisterNetEvent("client:bedleave")
