...
AddEventHandler("RefuelCarServerReturn",function()
    ...
    refillVehicle()
    exports["sway_taskbar"]:taskBar(timer, "Refueling")
    ...
    if car ~= 0 and (wasInCar or IsCar(car)) then
     ...
     if (GetPedInVehicleSeat(car, -1) == ped) then
         ...
         if IsControlJustReleased(0, Keys['M']) then
             if cruiseIsOn then
                 TriggerEvent('notification', 'Cruise Disabled', 1)
             else
                 TriggerEvent('notification', 'Cruise Activated', 1)
             end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsVehicleEngineOn(veh) then          
            ...
            if not exports["dsrp-inventory"]:hasEnoughOfItem("watch",1,false) then
                compass_on = false
            end
            ...
            if DoesEntityExist(veh) and IsEntityAVehicle(veh) and #(GetEntityCoords(veh) - GetEntityCoords(PlayerPedId())) < 5.0 then
                ...
                if IsControlJustPressed(1, 38) then
                    ...
                    if curFuel >= 100 then
                        ...
                        TriggerEvent('notification', 'You are already full!', 2)
                    elseif ESX.GetPlayerData().money > costs then
                        TriggerEvent("RefuelCarServerReturn")
                        TriggerServerEvent("carfill:pay",costs)
                    else
                        TriggerEvent('notification', 'Not enough money!', 2)
                    end
                end
            else
                Wait(100)
