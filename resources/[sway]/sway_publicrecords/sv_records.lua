...
RegisterServerEvent("esx-publicrecords:hotKeyOpen")
AddEventHandler("esx-publicrecords:hotKeyOpen", function()
    ...
    local xPlayer = esx.GetPlayerFromId(source)
    	...
    	MySQL.Async.fetchAll("SELECT * FROM (SELECT * FROM `mdt_reports` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(reports)
    		...
    		MySQL.Async.fetchAll("SELECT * FROM (SELECT * FROM `mdt_warrants` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(warrants)
    			...
    			TriggerClientEvent('esx-publicrecords:toggleVisibilty', usource, reports, warrants, officer)
    		end)
    	end)
    
end)

RegisterServerEvent("esx-publicrecords:getOffensesAndOfficer")
AddEventHandler("esx-publicrecords:getOffensesAndOfficer", function()
	local usource = source
	local charges = {}
	local jailtime = {}
	MySQL.Async.fetchAll('SELECT * FROM fine_types', {
		...
		TriggerClientEvent("esx-publicrecords:returnOffensesAndOfficer", usource, charges, officer)
	end)
end)

RegisterServerEvent("esx-publicrecords:performOffenderSearch")
AddEventHandler("esx-publicrecords:performOffenderSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.Async.fetchAll("SELECT * FROM `characters` WHERE LOWER(`firstname`) LIKE @query OR LOWER(`lastname`) LIKE @query OR CONCAT(LOWER(`firstname`), ' ', LOWER(`lastname`)) LIKE @query", {
		...
		TriggerClientEvent("esx-publicrecords:returnOffenderSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("esx-publicrecords:getOffenderDetails")
AddEventHandler("esx-publicrecords:getOffenderDetails", function(offender)
	local usource = source
	...
	MySQL.Async.fetchAll('SELECT * FROM `user_mdt` WHERE `char_id` = @id', {
		...
		MySQL.Async.fetchAll('SELECT * FROM `user_convictions` WHERE `char_id` = @id', {
			...
			MySQL.Async.fetchAll('SELECT * FROM `mdt_warrants` WHERE `char_id` = @id', {
				...
				TriggerClientEvent("esx-publicrecords:returnOffenderDetails", usource, offender)
			end)
		end)
	end)
end)

RegisterServerEvent("esx-publicrecords:getOffenderDetailsById")
AddEventHandler("esx-publicrecords:getOffenderDetailsById", function(char_id)
	local usource = source
...
RegisterServerEvent("esx-publicrecords:saveOffenderChanges")
AddEventHandler("esx-publicrecords:saveOffenderChanges", function(id, changes, identifier)
...
RegisterServerEvent("esx-publicrecords:saveReportChanges")
AddEventHandler("esx-publicrecords:saveReportChanges", function(data)
...
RegisterServerEvent("esx-publicrecords:deleteReport")
AddEventHandler("esx-publicrecords:deleteReport", function(id)
	MySQL.Async.execute('DELETE FROM `mdt_reports` WHERE `id` = @id', {
		['@id']  = id
	})
end)

RegisterServerEvent("esx-publicrecords:submitNewReport")
AddEventHandler("esx-publicrecords:submitNewReport", function(data)
	local usource = source
	...
	MySQL.Async.insert('INSERT INTO `mdt_reports` (`char_id`, `title`, `incident`, `charges`, `author`, `name`, `date`, `jailtime`) VALUES (@id, @title, @incident, @charges, @author, @name, @date, @sentence)', {
		...
		TriggerEvent("esx-publicrecords:getReportDetailsById", id, usource)
	end)
...
RegisterServerEvent("esx-publicrecords:sentencePlayer")
AddEventHandler("esx-publicrecords:sentencePlayer", function(jailtime, charges, char_id, fine, players)
	local usource = source
	local jailmsg = ""
	local xPlayer = esx.GetPlayerFromId(source)
			...
			MySQL.Async.fetchAll('SELECT * FROM `characters` WHERE `identifier` = @identifier', {
				...
				if result[1].id == char_id then
					if jailtime and jailtime > 0 then
						...
						TriggerEvent("esx-qalle-jail:jailPAVAClayer", src, jailtime, jailmsg)
					end
					return
				end
			end)
...
RegisterServerEvent("esx-publicrecords:performReportSearch")
AddEventHandler("esx-publicrecords:performReportSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.Async.fetchAll("SELECT * FROM `mdt_reports` WHERE `id` LIKE @query OR LOWER(`title`) LIKE @query OR LOWER(`name`) LIKE @query OR LOWER(`author`) LIKE @query or LOWER(`charges`) LIKE @query", {
		...
		TriggerClientEvent("esx-publicrecords:returnReportSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("esx-publicrecords:performVehicleSearch")
AddEventHandler("esx-publicrecords:performVehicleSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.Async.fetchAll("SELECT * FROM `owned_vehicles` WHERE LOWER(`plate`) LIKE @query", {
		...
		TriggerClientEvent("esx-publicrecords:returnVehicleSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("esx-publicrecords:performVehicleSearchInFront")
AddEventHandler("esx-publicrecords:performVehicleSearchInFront", function(query)
	local usource = source
    ...
    if xPlayer.job.name == 'police' then
		MySQL.Async.fetchAll("SELECT * FROM `owned_vehicles` WHERE `plate` = @query", {
			...
			TriggerClientEvent("esx-publicrecords:toggleVisibilty", usource)
			TriggerClientEvent("esx-publicrecords:returnVehicleSearchInFront", usource, result, query)
		end)
	end
end)

RegisterServerEvent("esx-publicrecords:getVehicle")
AddEventHandler("esx-publicrecords:getVehicle", function(vehicle)
	local usource = source
	MySQL.Async.fetchAll("SELECT * FROM `characters` WHERE `identifier` = @query", {
		...
		TriggerClientEvent("esx-publicrecords:returnVehicleDetails", usource, vehicle)
	end)
end)

RegisterServerEvent("esx-publicrecords:getWarrants")
AddEventHandler("esx-publicrecords:getWarrants", function()
	local usource = source
	MySQL.Async.fetchAll("SELECT * FROM `mdt_warrants`", {}, function(warrants)
		...
		TriggerClientEvent("esx-publicrecords:returnWarrants", usource, warrants)
	end)
end)

RegisterServerEvent("esx-publicrecords:submitNewWarrant")
AddEventHandler("esx-publicrecords:submitNewWarrant", function(data)
	local usource = source
	...
	MySQL.Async.insert('INSERT INTO `mdt_warrants` (`name`, `char_id`, `report_id`, `report_title`, `charges`, `date`, `expire`, `notes`, `author`) VALUES (@name, @char_id, @report_id, @report_title, @charges, @date, @expire, @notes, @author)', {
		...
		TriggerClientEvent("esx-publicrecords:completedWarrantAction", usource)
	end)
end)

RegisterServerEvent("esx-publicrecords:deleteWarrant")
AddEventHandler("esx-publicrecords:deleteWarrant", function(id)
	local usource = source
...
RegisterServerEvent("esx-publicrecords:getReportDetailsById")
AddEventHandler("esx-publicrecords:getReportDetailsById", function(query, _source)
	...
	MySQL.Async.fetchAll("SELECT * FROM `mdt_reports` WHERE `id` = @query", {
		...
		if result and result[1] then
			...
			TriggerClientEvent("esx-publicrecords:returnReportDetails", usource, result[1])
		end
	end)
end)
