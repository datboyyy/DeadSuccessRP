...
AddEventHandler('aqua:load', function(fromRegister)
        while not playerLoaded do
            Citizen.Wait(100)
        end

        if not fromRegister then
            TriggerServerEvent("clothing:checkIfNew")
        end

        if fromRegister then
...
RegisterNetEvent("openskinmenu")
AddEventHandler("openskinmenu", function (arg1, arg2, arg3)
...
function DisplayHelpText(str)
	...
	RegisterNetEvent('sway:outfit')
AddEventHandler('sway:outfit', function(args,action)
    if action == 1 then
        local id = args[2]
        table.remove(args, 1)
		table.remove(args, 1)
        ...
        TriggerEvent("raid_clothes:outfits", action, id, strng)
    elseif action == 2 then
        local id = args[2]
        TriggerEvent("raid_clothes:outfits", action, id)
...
RegisterCommand('outfits', function(source, args, rawCommand) 
        if args[1] == 'save' then
            if args[2] and args[3] and tonumber(args[2]) >= 0 and tonumber(args[2]) <= 10 then
                TriggerEvent('sway:outfit', args, 1)
                local notif = args[2]
                TriggerEvent('notification', 'Outfit Saved: ' ..notif)
            end
        elseif args[1] == 'remove' then
            if args[2] and not args[3] and tonumber(args[2]) >= 0 and tonumber(args[2]) <= 10 then
                TriggerEvent('sway:outfit', args, 2)
            end
        elseif args[1] == 'load' then
            if args[2] and not args[3] and tonumber(args[2]) >= 0 and tonumber(args[2]) <= 10 then
                TriggerEvent('sway:outfit', args, 3)
            end
        elseif args[1] == 'list' then
            TriggerEvent('sway:outfit')
        end
        if args[2] then
            if tonumber(args[2]) < 0 or tonumber(args[2]) > 10 then
                exports['mythic_notify']:SendAlert('inform', "Please choose an id between 0-10", 5000)
            end
        end
end, false)

Citizen.CreateThread(function()
TriggerEvent('chat:addSuggestion', '/outfits', 'save, remove, load or list outfits', {
    { name="action", help="save, remove, load, list" },
    { name="id", help="id of the outfit"},
    { name="name", help="name for the outfit"}
})
end)

RegisterNetEvent('raid_clothes:listskins')
AddEventHandler('raid_clothes:listskins', function(skincheck)
    if #skincheck == 0 then
        exports['mythic_notify']:SendAlert('inform', "You don't have any outfits/skins saved", 5000)
    else
	for i = 1, #skincheck do
        print(skincheck[i].slot .. " | " .. skincheck[i].name)
        TriggerEvent('notification', 'Outfit ID:' ..skincheck[i].slot .. " | Name:" .. skincheck[i].name, 1)
        end
    end 
end)
...
Citizen.CreateThread(function()
    ...
    for i=1, #clothingShops do
        local aqua1 = clothingShops[i]
        local clothingblip = AddBlipForCoord(aqua1[1], aqua1[2], aqua1[3])
        SetBlipSprite(clothingblip, 73)
        SetBlipDisplay(clothingblip, 4)
        SetBlipColour(clothingblip, 3)
        SetBlipScale(clothingblip, 0.7)
    ...
    for i=1, #barberShops do
        local aqua2 = barberShops[i]
        local barberblip = AddBlipForCoord(aqua2[1], aqua2[2], aqua2[3])
        SetBlipSprite (barberblip, 71)
        SetBlipDisplay(barberblip, 4)
        SetBlipColour(barberblip, 44)
        SetBlipScale(barberblip, 0.7)
    ...
    for i=1, #tattoosShops do
        local aqua3 = tattoosShops[i]
        local tattooblip = AddBlipForCoord(aqua3[1], aqua3[2], aqua3[3])
        SetBlipSprite(tattooblip, 75)
        SetBlipDisplay(tattooblip, 4)
        SetBlipColour(tattooblip, 1)
        SetBlipScale(tattooblip, 0.7)
        SetBlipAsShortRange(tattooblip, true)

        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString("Tattoo Artist")
