...
RegisterNUICallback('btnNotifyToggle', function(data, cb)
    ...
    if allowpopups then
      TriggerEvent("SendAlert","Popups Enabled", 1)
    else
      TriggerEvent("SendAlert","Popups Disabled", 2)
    end
end)
...
AddEventHandler('Yougotpaid', function(cidsent)
    if tonumber(cid) == tonumber(cidsent) then
      TriggerEvent("SendAlert","Life Invader Payslip Generated.", 1)
      TriggerEvent("SendAlert","Paycheck Added to your phone.", 1)
    end
end)

RegisterNetEvent('Yougotpaids')
AddEventHandler('Yougotpaids', function(cidsent, amount)
    if tonumber(cid) == tonumber(cidsent) then
      TriggerEvent("SendAlert", "$"..amount.." added to your payslip.", 1)
    end
end)
...
RegisterNetEvent("DSRP_housing:getid")
AddEventHandler("DSRP_housing:getid",function(house_ids)
	nearHousesID = house_ids
	 print('NEAR HOUSE ON PHONE', house_ids)
	-- print(json.encode(nearhouses))
end)
...
RegisterNUICallback('btnMortgage', function(data, cb)
  print('trigger here if any button in phone')
...
RegisterNUICallback('btnPropertyStorage', function(data, cb)
  ...
  print('this is housing storage')
  TriggerEvent('DSRP_housing:storage', nearHousesID)
...
RegisterNUICallback('btnAttempCreateHouse', function(data, cb)
  print('not creating')
  TriggerEvent("DSRP_housing:creatingHouse",data.name,data.price,data.garage,data.shell)
  --TriggerEvent("DSRP_housing:createHouse",data.cid,data.price)
end)
...
RegisterNUICallback('trackTaskLocation', function(data, cb)
    ...
    TriggerEvent("SendAlert","Location Set", 1)
...
AddEventHandler("phone:error", function()
      SendNUIMessage({
        openSection = "error",
        textmessage = "<b>Network Error</b> <br><br> Please contact support if this error persists, thank you for using DSRP Phone Services.",
      })   
end)
...
RegisterCommand("coolpeopleonly", function(source, args, rawCommand)
  TriggerEvent("open:deepweb")
end, false)
...
RegisterNUICallback('btnMute', function()
  if phoneNotifications then
    TriggerEvent("SendAlert","Notifications Disabled.", 2)
  else
    TriggerEvent("SendAlert","Notifications Enabled.", 1)
  end
    ...
    for i = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
            if Trim(GetVehicleNumberPlateText(vehicle)) == plate then
                ...
                if phoneNotifications then
                  TriggerEvent("SendAlert","Your vehicle has been marked on your GPS.", 1)
...
RegisterNetEvent('GPS:SetRoute2')
AddEventHandler('GPS:SetRoute2', function(x,y,z)
	print(house_id, house_model)
	if GPSblip ~= nil then
		RemoveBlip(GPSblip)
  end
  print(x,y,z)
	GPSblip = AddBlipForCoord(x,y,z)
	TriggerEvent("GPSActivated",true)
	SetBlipRoute(GPSblip, 1)
	SetBlipAsFriendly(GPSblip, 1)
	SetBlipColour(GPSblip, 6)
	TriggerEvent("SendAlert","Your GPS location has been set to your house!")
...
AddEventHandler("phone:Garage", function(vehs)
  ...
  for ind, value in pairs(vehs) do
    ...
    local financetimer = value.repaytime
...
--   -- print('this is button trucker shit')
--   TriggerEvent("Trucker:GetJobs")
--   TriggerServerEvent("trucker:returnCurrentJobs")
--   TriggerServerEvent("trucker:returnCurrentJobs1")
-- end)
...
RegisterNUICallback('resetPackages', function()
  ...
  --     JobType = "Delivery",
  --     active = 0,
  --     pickup   = {"2747.82", "3454.10", "56.01" }, 
  --     drop = { x = "235.23", y = "163.93", z = "105.13" },
  --     dropAmount = 8,
  --   },
  ...
  --     active = 0,
  --     pickup   = {"2747.82", "3454.10", "56.01" }, 
  --     drop = { x = "500.024", y = "-676.52", z = "33.04" },
  --     dropAmount = 8,
  --   }
  -- }
...
RegisterNUICallback('selectedJob', function(data, cb)
  --  print(json.encode(data), json.encode(cb))
...
function hasDecrypt2()
  if exports["dsrp-inventory"]:hasEnoughOfItem("vpnxj",1,false) and not exports["isPed"]:isPed("disabled") then
    return true
  else
    return false
  end
...
AddEventHandler('canPing', function(target)
  if hasPhone() and not isDead then
    ...
    TriggerEvent("SendAlert","You need a phone to use GPS!",2)
  end
end)
...
AddEventHandler('allowedPing', function(x,y,z,src,name)
  if pingcount > 0 then
    TriggerEvent("SendAlert","Somebody sent you a GPS flag but you already have one in process!",2)
    return
  end
  ...
  if hasPhone() and not isDead then
    ...
    while pingcount > 0 do
      TriggerEvent("SendAlert",name .. " has given you a ping location, type /pingaccept to accept",2)
      Citizen.Wait(5000)
      pingcount = pingcount - 1
    end
  else
    TriggerEvent("SendAlert","Somebody sent you a GPS flag but you have no phone!",2)
  end
  pingcount = 0
...
AddEventHandler('acceptPing', function()
  if pingcount > 0 then
    ...
    TriggerEvent("SendAlert","Their GPS ping has been marked on the map")
...
Citizen.CreateThread(function()
  while true do
      Citizen.Wait(1)
    if IsDisabledControlJustPressed(1, 199) then	--SHIFT AND P
      openGuiNow()
    end
  end
end)
...
function hasPhone()
  ...
  if
    (
    (exports["dsrp-inventory"]:hasEnoughOfItem("mobilephone",1,false) or 
    exports["dsrp-inventory"]:hasEnoughOfItem("stoleniphone",1,false) or 
    exports["dsrp-inventory"]:hasEnoughOfItem("stolens8",1,false) or
    exports["dsrp-inventory"]:hasEnoughOfItem("stolennokia",1,false) or
    exports["dsrp-inventory"]:hasEnoughOfItem("stolenpixel3",1,false) or
    exports["dsrp-inventory"]:hasEnoughOfItem("boomerphone",1,false)or
    exports["dsrp-inventory"]:hasEnoughOfItem("assphone",1,false))
...
RegisterNetEvent('tp:heHasVPN')
AddEventHandler('tp:heHasVPN', function(option)
 decrypt2 = option
end)
...
function openGuiNow()
  ...
  if hasPhone() then
    ...
    if not exports["isPed"]:isPed("disabled") then
      TriggerEvent("SendAlert","You do not have a phone.",2)
    else
      TriggerEvent("SendAlert","You cannot use your phone right now.",2)
    end
  end
  recentopen = false
end
...
RegisterNUICallback('newPostSubmit', function(data, cb)
  ...
  if ESX.PlayerData.job.name ~= "taxi" and ESX.PlayerData.job.name ~= "towtruck" and ESX.PlayerData.job.name ~= "trucker" then
    ...
    TriggerEvent("SendAlert","You are already listed as a " .. myjob)
  end
end)

RegisterNUICallback('newCPostSubmit', function(data, cb)
  TriggerServerEvent('phone:SubmitMsgCP', data.advert)
end)
...
RegisterNetEvent("close:Phone")
AddEventHandler("close:Phone", function()
  closeGui()
end)
...
AddEventHandler("yellowPages:retrieveLawyersOnline", function(command)
    ...
    TriggerEvent("SendAlert","Searching for a Lawyer", 1)
    Citizen.Wait(2000)
...
RegisterNUICallback('criminalPage', function()
  print('this is criminal page')
  TriggerServerEvent('getCP')
  Citizen.Wait(200)
  TriggerEvent("CPUpdatePhone")
  TriggerServerEvent('getNM')
end)
...
RegisterNetEvent('CPUpdatePhone')
AddEventHandler('CPUpdatePhone', function()

  lstnotifications = {}

  local jsonparse = json.decode(CriminalPages)

  for i = 1, #CriminalPageArray do
    lstnotifications[#lstnotifications + 1] = {
      id = tonumber(i),
      name = jsonparse[tonumber(i)].name,
      message = jsonparse[tonumber(i)].advert,
  ...
  SendNUIMessage({openSection = "criminalPage", list = lstnotifications})
end)
...
RegisterNUICallback('newMessageSubmit', function(data, cb)
  if not isDead then
    ...
    TriggerEvent("SendAlert","You can not do this while injured.",2)
  end
end)
...
RegisterNetEvent('refreshCP')
AddEventHandler('refreshCP', function()
  if guiEnabled then
    TriggerServerEvent('getCP')
    Citizen.Wait(250)
    TriggerEvent('CPUpdatePhone')
...
AddEventHandler('animation:phonecallstart', function()
  ...
  TriggerEvent("SendAlert","[E] Toggles Call.", 1)
  ...
  while callStatus ~= isNotInCall do
    ...
    if AnonCall then
       ...
       if dPB > 2.0 then
        TriggerEvent("SendAlert","Moved too far.", 2)
        endCall()
       end
    end
    ...
    if onhold then
      if count == 800 then
         count = 0
         TriggerEvent("SendAlert","Call On Hold.", 1)
      end
    end
...
AddEventHandler('phone:makepayphonecall', function(pnumber) 
    ...
    if not checkFkrpayPhone() then
      TriggerEvent("SendAlert","You are not near a payphone.",2)
      return
    end
    ...
    if callStatus == isNotInCall and not isDead then
      ...
      TriggerEvent("SendAlert","It appears you are already in a call, injured or with out a phone, please type /hangup to reset your calls.",2)
    end
...
RegisterNUICallback('deleteCPpost', function(data, cb)
  -- print('this is delete',data.number, data.adv)
  TriggerServerEvent('phone:deleteCP',data.number,data.adv)
end)
...
AddEventHandler('phone:otherClientEndCall', function()
    ...
    TriggerEvent("SendAlert", "Your call was ended!", 2)
    callid = 0
    myID = 0
    mySourceID = 0
    mySourceHoldStatus = false
...
AddEventHandler('phone:answercall', function()
    ...
    answerCall()
    TriggerEvent("animation:phonecallstart")
    TriggerEvent("SendAlert","You have answered a call.", 1)
    callTimer = 0
  else
    TriggerEvent("SendAlert","You are not being called, injured, or you took too long.", 2)
  end
end)

RegisterNetEvent('phone:initiateCall')
AddEventHandler('phone:initiateCall', function(srcID)
  TriggerEvent("SendAlert","You have started a call.", 1)
    initiatingCall()
...
AddEventHandler('phone:addToCall', function(voipchannel)
  ...
  exports["mumble-voip"]:SetCallChannel(voipchannel + 1)
end)
...
AddEventHandler('phone:callactive', function()
    ...
    while callStatus == isCallInProgress do
      ...
      if onhold then
        ...
        if not held1 then
          TriggerEvent("SendAlert","You have put the caller on hold.",888)
          held1 = true
        end
      else
        phoneString = phoneString .. "Call Active | "
        if held1 then
          TriggerEvent("SendAlert","Your call is no longer on hold.",888)
          held1 = false
        end
      end
      ...
      if mySourceHoldStatus then
        ...
        if not held2 then
          TriggerEvent("SendAlert","You are on hold.",2)
          held2 = true
        end
      else
        phoneString = phoneString .. "Caller Active"
        if held2 then
          TriggerEvent("SendAlert","You are no longer on hold.",2)
          held2 = false
        end
      end
...
AddEventHandler('phone:receiveCall', function(phoneNumber, srcID, calledNumber)
 -- local callFrom = getContactName(calledNumber)
  ...
  if callStatus == isNotInCall then
    ...
    TriggerEvent("SendAlert","You are receiving a call from " .. callFrom .. " but are currently already in one, sending busy response.",2)
  end
end)
callTimer = 0
function initiatingCall()
  callTimer = 8
  TriggerEvent("SendAlert","You are making a call, please hold.", 1)
...
function receivingCall(callFrom)
  ...
  while (callTimer > 0 and callStatus == isReceivingCall) do
    if hasPhone() then
      TriggerEvent("SendAlert","Call from: " .. callFrom .. " /answer | /hangup",10)
...
AddEventHandler('phone:holdToggle', function()
  ...
  if myID ~= 0 then
    if not onhold then
      ...
      TriggerEvent("SendAlert","Call on hold", 2)
      ...
      TriggerEvent("SendAlert","No longer on hold.", 1)
...
AddEventHandler('OnHold:Client', function(newHoldStatus,srcSent)
    ...
    if mySourceHoldStatus then
        ...
        TriggerEvent("SendAlert","You just got put on hold.")
    else
        if not onhold then
          local playerId = GetPlayerFromServerId(srcSent)
          
        end
        TriggerEvent("SendAlert","Your caller is back on the line.")
    end
end)
----------
...
RegisterNetEvent('CriminalPageArray')
AddEventHandler('CriminalPageArray', function(pass)
    local notdecoded = json.encode(pass)
    CriminalPages = notdecoded

    CriminalPageArray = pass
end)
...
AddEventHandler('Client:UpdateTweet', function(tweet)
    ...
    if string.find(currentTwats[#currentTwats]["message"],handle) then
      ...
      if phoneNotifications then
        PlaySound(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0, 0, 1)
        TriggerEvent("SendAlert","You were just mentioned in a tweet on your phone.",15)
      end
    end
...
RegisterNUICallback('loadGPS', function()
  TriggerEvent("SendAlert","Coming soon.", 2)
  TriggerEvent("openGPS")
end)

RegisterNUICallback('loadHouseSell', function()
  TriggerEvent("SendAlert","Coming soon.", 2)
  TriggerEvent("openHouseSell")
end)
...
AddEventHandler('openGPS', function(mansions,house,rented)
  local house = {
    -- [23] = { ["x"] = -25.59, ["y"] = -720.86, ["z"] = 32.22, ["info"] = "Garage House"},
    { ["x"] = 92.67, ["y"] = 49.43, ["z"] = 73.52, ["info"] = "Housing 1"}
  }
  if loadedGPS then
  ...
  local mapLocationsObject = {
    ...
    houses = { info = house, houseType = 1, x = x, y = y, z = z },
...
local loadedHouseSell = false
RegisterNetEvent('openHouseSell')
AddEventHandler('openHouseSell', function(mansions,house,rented)
  local house = {
    -- [23] = { ["x"] = -25.59, ["y"] = -720.86, ["z"] = 32.22, ["info"] = "Garage House"},
    { ["x"] = -397.99609375, ["y"] = 175.88108825684, ["z"] = 103.85623168945, ["info"] = "Housing 1"}
  }
  if loadedGPS then
    SendNUIMessage({openSection = "houseSell"})
    return
  end
  local mapLocationsObject = {
    custom = { info = customGPSlocations, houseType = 69 },
  ...
  SendNUIMessage({openSection = "houseSell", locations = mapLocationsObject })
  loadedGPS = true
end)

local myHandlerTwatte = ""


RegisterNUICallback('loadUserGPS', function(data)
  print('HOUSING',json.encode(data))
    TriggerEvent("GPS:SetRoute2",data.x,data.y,data.z)
end)
...
local havePaycheck = false
RegisterNUICallback('payslip', function()
  if not havePaycheck then
      local info = {
        payslip = {},
        total = 0
      }
      SendNUIMessage({openSection = "paySlip", response = info})
  else
    TriggerServerEvent('server:paySlipPickup')
  end
end)

RegisterNetEvent('phone:payslip')
AddEventHandler('phone:payslip',function(payslip,total)
  havePaycheck = true
  local info = {
    payslip = payslip,
    total = total
...
---Paycheck button
RegisterNUICallback('phone:collectPaychek', function()
  Wait(1000)
  closeGui()
  havePaycheck = false
  getMycheck()
...
function getMycheck()
  ...
  if payDst < 3.5 then
    TriggerServerEvent('phone:server:collect:paycheck')
  else
    TriggerEvent("SendAlert", "Error! You are not in Live Invader. Go there!", 2)
    havePaycheck = true
  end
end
...
AddEventHandler('phone:newSMS', function(id, number, message, mypn, date, recip)
  ...
  if hasPhone() then
    SendNUIMessage({
      ...
      TriggerServerEvent('phone:getSMS') 
    if phoneNotifications then
      TriggerEvent("SendAlert", "You just received a new SMS.", 1)
      PlaySound(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0, 0, 1)
    end
  end
end)
...
--print(map,laps,counter,reverseTrack,raceName, startTime, mapCreator, mapDistance, mapDescription)
...
function RunRace(identifier)
  ...
  TriggerEvent("SendAlert","Race Starts in 3",14)
  PlaySound(-1, "3_2_1", "HUD_MINI_GAME_SOUNDSET", 0, 0, 1)
  Citizen.Wait(1000)
  TriggerEvent("SendAlert","Race Starts in 2",14)
  PlaySound(-1, "3_2_1", "HUD_MINI_GAME_SOUNDSET", 0, 0, 1)
  Citizen.Wait(1000)
  TriggerEvent("SendAlert","Race Starts in 1",14)
  PlaySound(-1, "3_2_1", "HUD_MINI_GAME_SOUNDSET", 0, 0, 1)
  Citizen.Wait(1000)
  PlaySound(-1, "3_2_1", "HUD_MINI_GAME_SOUNDSET", 0, 0, 1)
  TriggerEvent("SendAlert","GO!",14)
  hudUpdate('start', {
    isSprint = sprint,
    maxLaps = laps,
  ...
  PlaySound(-1, "3_2_1", "HUD_MINI_GAME_SOUNDSET", 0, 0, 1)
  TriggerEvent("SendAlert","You have finished!",14)
  Wait(10000)
  racing = false
  hudUpdate('clear')
  ClearBlips()
...
function SaveMap(name,description)
  ...
  if #currentMap > 1 then
    TriggerEvent("SendAlert","The map is being processed and should be available shortly.",2)
    TriggerServerEvent("racing-save-map",currentMap,name,description,distanceMap)
  else
    TriggerEvent("SendAlert","Failed due to lack of checkpoints",2)
  end
  currentMap = {}
  creatingMap = false
end
