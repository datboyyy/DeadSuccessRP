...
RegisterNUICallback('assistambulance', function(data, cb)
  TriggerEvent('phone:assistRequest', 'ambulance', data.message)
  cb('ok')
end)
...
AddEventHandler('phone:assistResponse', function(service, message)
  --TriggerEvent("DoShortHudText", "Assistance is on its way!",1)
end)

RegisterNetEvent('phone:assistNotify')
AddEventHandler('phone:assistNotify', function(id, service, player)
  TriggerEvent('jobssystem:current', function(job)
    if (job == 'ambulance' and service == 'AIcall7') then
      addBlip({service = service, player = player, id = id,jobType = "ambulance"})
      TriggerEvent("chatMessage", "DISPATCH ", 3, 'A vehicular accident has been reported!')
    end
    if (job == 'ambulance' or job == 'news') and (service == 'ambulance' or service =='AIcall23') or (job == 'ambulance' and service == 'AIcall4') then
      if job == 'news' then
...
AddEventHandler('phone:assistRequest', function(service, message)
  if(service ~= nil and message ~= nil) then
    ...
    --TriggerEvent("DoShortHudText", "You must fill in a service to request and message!",2)
  end
end)
...
AddEventHandler('phone:assistPayJ', function(name)
    ...
    if name == 'taxi' then
      ...
      if driverPed then
        if IsPedInAnyTaxi(ped) then
          ...
          --TriggerEvent("DoShortHudText","Please use a taxi or sign off duty.",2)
        end   
      end
    ...
    elseif name == 'news' then
      ...
      --TriggerEvent("DoShortHudText","Payment added to next payout.",2)
  ...
  while true do
    ...
    elseif exports["np-base"]:getModule("LocalPlayer"):getVar("job") == "news" then
      ...
      if holding and dist < 130 and dist ~= 0 and blipcount >= 1 then
        ...
        newsPayment = 0
        blipcount = 0 
        --TriggerEvent("DoShortHudText","You have been paid a bonus for your dedication.",2)
      else
    ...
    for key, item in pairs(myBlips) do
      if (key ~= nil and item ~= nil) then
        ...
        if #(pos - vector3( item.pos.x, item.pos.y, item.pos.z)) < 50.0001 then
          if item.jobType == "ambulance" then
            if exports["np-base"]:getModule("LocalPlayer"):getVar("job") == "ambulance" then
