...
AddEventHandler("checkifbike", function(vehicle, veh_id)
	...
	if bicycle then
		...
		TriggerEvent( "notification", "No License.", 2 )
	end
end)
...
Citizen.CreateThread(function() --ok
	...
	while ESX == nil do
	  TriggerEvent('esx:getShAVACaredObjAVACect', function(obj) ESX = obj end)
	  Citizen.Wait(1)
	end
					...
					if #(vector3(storage["x"],storage["y"],storage["z"]-0.3) - plyCoords) < 1.5 then
						...
						if IsControlJustReleased(2, 38) then
							if row["house_owner_cid"] == exports["isPed"]:isPed("cid") then
								...
								TriggerEvent("notification", "You are not allowed to open other's stashes.", 1)
							end
						end
					end
				end
...
local garage = {title = "garage", currentpos = nil, marker = { r = 222, g = 50, b = 50, a = 1, type = 1 }}
...
function spawnCars(houseCars)
 ...
 function ListeVehicule(page)
	...
	ClearMenu()
	local carCount = 0
	--ESX.TriggerServerCallback("garage:fetchPlayerVehicles", function(playerVehicles)
    --    for _, value in pairs(playerVehicles) do
    --    --carCount = carCount + 1
    --    print("after callback")
	--	end 
	--end, current_used_garage)
...
ESX.TriggerServerCallback("garage:fetchPlayerVehicles", function(playerVehicles)
	...
	for _, value in pairs(playerVehicles) do
	   ...
	   local vehicleProps = value["props"]
	   local koy = value["id"]
	   local vehicleName  = value["model"]

	   enginePercent = vehicleProps.engineHealth / 10
	   bodyPercent = vehicleProps.bodyHealth / 10
	   garage = value.garage
		plate = value.plate
		local replace = string.gsub (garage, garage, "In")
		 ...
		 if garage == nil then
			 garage = "Any"
				...
				if vehicle_state == "Standard Impound" then
					Menu.addButton(tostring(vehicleName), "OptionVehicle", koy, "Impounded: $500", " Engine %:" .. ESX.Math.Round(enginePercent,1) .. "", " Body %:" .. ESX.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
				elseif vehicle_state == "Police Impound" then
					Menu.addButton(tostring(vehicleName), "OptionVehicle", koy, "Impounded: $1500", " Engine %:" .. ESX.Math.Round(enginePercent,1) .. ""," Body %:" .. ESX.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
				elseif garage ~= current_used_garage and garage ~= "Any" and vehicle_state ~= "Out" then
					Menu.addButton(tostring(vehicleName), "WhoYouKiddingBoi", koy,  "Out", " Engine %:" .. ESX.Math.Round(enginePercent,1) .. ""," Body %:" .. ESX.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
				else
					Menu.addButton(tostring(vehicleName), "OptionVehicle", koy, replace, " Engine %:" .. ESX.Math.Round(enginePercent,1) .. ""," Body %:" .. ESX.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
				end
			end
		--end
	   count = count + 1
   ...
   if page == 0 then
    ...
    TriggerEvent("notification", "It will cost $" .. estimate .. " to $" .. estimate + (estimate * 0.15) .. " @ $15 x (carCount x carCount x 2)",1)
...
function WhoYouKiddingBoi()
	--local vehID = 1
	MenuTitle = "Options :"

	ClearMenu()
	TriggerEvent('notification', 'THIS VEHICLE IS OUT!',1)
...
function OptionVehicle(vehID)
	...
	ClearMenu()
	if vehID == 0 then
		TriggerEvent('notification', 'You dont have vehicle in this garage')
		Menu.addButton("Return", "ListeVehicule", nil)
	else
...
function SortirVehicule(vehID)
	...
	if current_used_garage == "Impound Lot" then
		...
		TriggerServerEvent('garages:CheckGarageForSpawn', vehID, garageCost)
	else
		local garageCost = 0
	TriggerServerEvent('garages:CheckGarageForSpawn', vehID, garageCost)
	end
				...
				while garagesRunning do
					...
					if dist < 35.0 then
						...
						if myroomtype ~= 3 and gar == "House" then
							...
							DrawMarker(20,pos[1],pos[2],pos[3],0,0,0,0,0,0,0.701,1.0001,0.3001,markerColor.Yellow,markerColor.Red, markerColor.Blue,dist,0,0,0,0)
						end
					end
...
AddEventHandler("garages:getVehicles", function(ownedCars)
	ESX.TriggerServerCallback('garages:getOwnedCars', function(ownedCars)
		if #ownedCars == 0 then
...
AddEventHandler('garages:SpawnVehicle', function(car, plate, garage, vehicle, vehID)

	----print('PIZDA')
	--print(car)
	--print(plate)
	--print(garage)
	--print(vehicle["fuelLevel"])
	--print(vehID)
	...
	Citizen.CreateThread(function()			
		...
		if DoesEntityExist(caisseo) then
			...
			TriggerEvent("notification", "The area is crowded",2)

		else
			if garage == "Out" then
				TriggerEvent("notification","Not in garage",2)
			else	

				if garage == "Normal Impound" then
					TriggerEvent("notification","This vehicle cost you $100.",1)
				end

				if garage == "Police Impound" then
					TriggerEvent("notification","This vehicle cost you $1500.",1)
				end
				...
				local spawnPos = garages[selectedGarage].spawn
					...
					if coordlocation ~= nil then
						...
						veh = SpawnVehicle(car, plate, fuelSet, coordlocation[1],coordlocation[2],coordlocation[3], 0.0)
						TriggerServerEvent('garage:addKeys', plate)
		
					else
					
					print(plate)
						--veh = SpawnVehicle(car, plate, vehicle["fuelLevel"], spawnPos[1], spawnPos[2], spawnPos[3], spawnPos[4], vehID, vehicle)
...
function SpawnVehicle(vehicle, plate, fuelSet, spawnx, spawny, spawx, spawnz, vehiD, vehicle_prop)
	...
	ESX.Game.SpawnVehicle(vehicle, {
		...
		--print('je')
		SetVehicleProperties(callback_vehicle, vehicle_prop, fuelSet)
		doCarDamages(vehicle_prop.engineHealth, vehicle_prop.bodyHealth, fuelSet, callback_vehicle)
		NetworkFadeInEntity(callback_vehicle, true, true)
		SetModelAsNoLongerNeeded(vehicle_prop.model)
		...
		TriggerServerEvent('garages:SetVehOut', plate, vehiD)
	end)
...
AddEventHandler('garages:StoreVehicle', function()
    ...
    if vehicle == 0 then
		TriggerEvent('notification', 'No vehicle')
		...
		ESX.TriggerServerCallback("garages:checkOwnerPlate", function(bol)
			...
			if bol == true then
				...
				local vehId = GetHashKey(vehicle)
				...
				TriggerEvent('notification', 'You saved your vehicle in garage '..current_used_garage)
				...
				--TriggerServerEvent('garages:SetVehIn', plate, current_used_garage, vehicleProps, livery, realFuel, type, key, vehId)
				TriggerServerEvent('garages:SetVehInIn', plate, vehicleProps, current_used_garage)
				CloseMenu()
--			
	...
	Citizen.CreateThread(function()		
		...
		if DoesEntityExist(caissei) then	
			if plate ~= platecaissei then					
				TriggerEvent("notification","It's not your vehicle",2)
			else
			...
			TriggerEvent("notification","No Vehicle",2)
		end   
	end)
end)
...
AddEventHandler('garages:SellToPlayer', function(price,plate,player)
	...
	Citizen.CreateThread(function()		
		...
		if not DoesEntityExist(GetPlayerPed(targetPed)) then
			TriggerEvent("notification","No Person",2)
			return
		end
		...
		if #(vector3(pos.x,pos.y,pos.z) - vector3(pos2.x,pos2.y,pos2.z)) < 40 then
			...
			if DoesEntityExist(caissei) then	
				if plate ~= platecaissei then					
					...
					if isWanting then
						...
						TriggerEvent("notification","Person has declined.",2)
					end
				end
			else
				TriggerEvent("notification","No Vehicle",2)
			...
			TriggerEvent("notification","Not close enough.",2)
		end
	end)
end)
