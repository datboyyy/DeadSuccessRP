...
ESX.RegisterServerCallback('garages:getOwnedCars', function(source, cb)
	local ownedCars = {}
...
AddEventHandler("garages:PayedImpoundCar",function(plate)
	...
	TriggerClientEvent('notification', src, 'You payed $700 for this transaction with tax. Please always check your email for car delivery.')
	Citizen.Wait(60000)
	--TriggerClientEvent('notification',src, 'Your car is delivered in Garage T.')
...
AddEventHandler('garages:CheckForSpawnVeh', function(veh_id, garageCost)
  ...
  AddEventHandler('garages:CheckForVeh', function()
	...
	local user = ESX.GetPlayerFromId(source)
	  ...
	  TriggerClientEvent('garages:StoreVehicle', src, plate)
			...
			TriggerClientEvent('notification', src, 'Theres no car or is not yours', 2)
		end
  ...
  AddEventHandler('onResourceStart', function(resource)
	--print("THIS IS FOR SAVING YOUR LIFE")
	if GetCurrentResourceName() == 'garages' then
...
function ImpoundCarOut()
	Citizen.CreateThread(function()
  MySQL.Async.execute("UPDATE owned_vehicles SET garage = @garage, state = @state WHERE state = @state", {['garage'] = 'Impound Lot', ['state'] = 'Nomal Impound', ['state'] = 'Out'})
  Citizen.Wait(1000)
  MySQL.Async.execute("UPDATE owned_vehicles SET state = @state WHERE garage = @garage", {['state'] = 'Nomal Impound', ['garage'] = 'Impound Lot'})
	end)
end