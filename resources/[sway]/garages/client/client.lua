			...
			if enableHgarage then
				...
				current_used_garage = houseShell..'-'..houseID
					...
					if vehicle_state == "Standard Impound" then
						Menu.addButton(tostring(vehicleName), "OptionVehicle", value.id, "Impounded: $500", " Engine %:" .. ESX.Math.Round(enginePercent,1) .. "", " Body %:" .. ESX.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
					elseif vehicle_state == "Police Impound" then
						Menu.addButton(tostring(vehicleName), "OptionVehicle", value.id, "Impounded: $1500", " Engine %:" .. ESX.Math.Round(enginePercent,1) .. ""," Body %:" .. ESX.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
					elseif curGarage ~= current_used_garage and curGarage ~= "Any" and vehicle_state ~= "Out" and curGarage ~= "house" then
						
						Menu.addButton(tostring(vehicleName), "OptionVehicle", 0, curGarage, " Engine %:" .. ESX.Math.Round(enginePercent,1) .. "", " Body %:" .. ESX.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
		
					else
					
						Menu.addButton(tostring(vehicleName), "OptionVehicle", value.id, tostring(value.state) , " Engine %:" .. ESX.Math.Round(enginePercent,1) .. ""," Body %:" .. ESX.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
					end
				end
			--end
		   count = count + 1
...
function SpawnVehicle(vehicle, plate, fuelSet, spawnx, spawny, spawx, spawnz)
	ESX.Game.SpawnVehicle(vehicle.model, {
		...
		TriggerServerEvent('garage:addKeys', plate)
		TriggerEvent("keys:addNew",callback_vehicle,GetVehicleNumberPlateText(callback_vehicle))
...
AddEventHandler('garages:StoreVehicle', function()
	ClearMenu()
	local nearestcar = GetClosestVehicle(PlayerPedId())
	...
	if vehicle == nil then 
        ...
        CloseMenu()
			...
			TriggerServerEvent('garages:SetVehIn', plate, current_used_garage, vehicleData, livery, realFuel)
			CloseMenu()
				end
