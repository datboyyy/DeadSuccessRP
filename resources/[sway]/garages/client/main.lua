...
RegisterNetEvent("CityLife:NCPP:Garage")
AddEventHandler("CityLife:NCPP:Garage", function()
    TriggerServerEvent("NCPP:GarageExploit")
end)
		...
		if #ownedCars == 0 then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_car', {
				...
				if data.current.value.stored then
					...
					exports['mythic_notify']:SendAlert('inform', _U('car_is_impounded'), 5000)
				end
			end, function(data, menu)
				menu.close()
			end)
...
function ListOwnedBoatsMenu()
	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedBoats', function(ownedBoats)
		if #ownedBoats == 0 then
			exports['mythic_notify']:SendAlert('error', _U('garage_noboats'), 5000)
		else
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_boat', {
				...
				if data.current.value.stored then
					...
					exports['mythic_notify']:SendAlert('inform', _U('boat_is_impounded'), 5000)
				end
			end, function(data, menu)
				menu.close()
			end)
...
function ListOwnedAircraftsMenu()
	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedAircrafts', function(ownedAircrafts)
		if #ownedAircrafts == 0 then
			exports['mythic_notify']:SendAlert('error', _U('garage_noaircrafts'), 5000)
		else
		...
		css = 'garage',
			...
			if data.current.value.stored then
				...
				exports['mythic_notify']:SendAlert('error', _U('aircraft_is_impounded'), 5000)
			end
		end, function(data, menu)
			menu.close()
		end)
...
--				end	
				...
				exports['mythic_notify']:SendAlert('error', _U('cannot_store_vehicle'), 5000)
			end
		end, vehicleProps)
	else
		exports['mythic_notify']:SendAlert('error', _U('no_vehicle_to_enter'), 5000)
	end
end
				...
				if not onCoolDown then
					ESX.TriggerServerCallback('esx_advancedgarage:checkMoneyCars', function(hasEnoughMoney)
						if hasEnoughMoney then
							...
							exports['mythic_notify']:SendAlert('error', _U('not_enough_money'), 5000)
						end
					end)	
				else
					exports['mythic_notify']:SendAlert('error', 'Please wait 5 seconds before bringing your vehicle out', 5000)
				end
			end, function(data, menu)
				menu.close()
			end)
...
function reparation(apprasial, vehicle, vehicleProps)
	...
	local elements = {
		{label = "Store Vehicle", value = 'yes'},
		{label = "Repair Vehicle" .." ($"..apprasial..")", value = 'no'}
	}
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'delete_menu', {
		...
		if data.current.value == 'yes' then -- Store Vehicle Damaged

			exports['mythic_notify']:SendAlert('inform', 'Vehicle Stored Damaged', 5000)
			putaway(vehicle, vehicleProps)
		elseif data.current.value == 'no' then -- Pay to repair + store it
...
function putaway(vehicle, vehicleProps)
	...
	TriggerServerEvent('esx_advancedgarage:setVehicleState', vehicleProps.plate, true, vehicleProps)
	TriggerServerEvent('esx_advancedgarage:store', vehicleProps.plate, vehicleProps)

	
	exports['mythic_notify']:SendAlert('inform', _U('vehicle_in_garage'), 5000)
end
--[[
Getting	{"pearlescentColor":0,"plateIndex":0,"neonColor":[255,0,255],"xenonheadlights":8,"modExhaust":-1,
"modSideSkirt":-1,"modRearBumper":-1,"bodyHealth":1000.0,"modHydrolic":-1,"modTrimB":-1,"modHood":-1,"modAirFilter":-1,"modDial":-1,"modGrille":-1,"modSmokeEnabled":1,
"modTank":-1,"modAPlate":-1,"modEngineBlock":-1,"modDashboard":-1,"modArchCover":-1,"modFender":-1,"engineHealth":1000.0,"modVanityPlate":-1,"modSpoilers":-1,"model":-431692672,
"modRightFender":-1,"interiourcolor":0,"modHorns":-1,"modStruts":-1,"color1":111,"modFrontBumper":-1,"modTurbo":false,"modBackWheels":-1,"modFrame":-1,"color2":111,"wheels":5,
"extras":{"3":true,"2":true,"5":true,"4":true,"1":true},"neonEnabled":[false,false,false,false],"modShifterLeavers":-1,"modSuspension":-1,"modBrakes":-1,"modEngine":-1,
"modAerials":-1,"modPlateHolder":-1,"modSteeringWheel":-1,"windowTint":-1,"modTrimA":-1,"tyreSmokeColor":[255,255,255],"modTrunk":-1,"plate":"ZENU0864","modArmor":-1,
"modOrnaments":-1,"modSeats":-1,"modLivery":-1,"dirtLevel":0.1,"modWindows":-1,"modRoof":-1,"modDoorSpeaker":-1,"wheelColor":156,"modFrontWheels":-1,"modTransmission":-1,
"modXenon":1,"modSpeakers":-1}


Setting vehicle props	{"pearlescentColor":0,"plateIndex":0,"neonColor":[255,0,255],"modPlateHolder":-1,"modExhaust":-1,"modSideSkirt":-1,"modFrontBumper":-1,
"bodyHealth":865.8,"modHydrolic":-1,"modRearBumper":-1,"modHood":-1,"modAirFilter":-1,"modDial":-1,"modStruts":-1,"modSteeringWheel":-1,"modTank":-1,"modAPlate":-1,
"modShifterLeavers":-1,"modDashboard":-1,"modFender":-1,"modWindows":-1,"engineHealth":351.7,"modVanityPlate":-1,"modSpoilers":-1,"model":-431692672,"modSmokeEnabled":1,
"interiourcolor":0,"modHorns":-1,"xenonheadlights":8,"color1":111,"modTrimA":-1,"modFrame":-1,"modBackWheels":-1,"modTrunk":-1,"modXenon":1,"wheels":5,
"extras":{"3":true,"2":true,"5":true,"4":true,"1":true},"neonEnabled":[false,false,false,false],"modSeats":-1,"modSuspension":-1,"modRightFender":-1,"modEngine":-1,
"modAerials":-1,"modArchCover":-1,"modGrille":-1,"modBrakes":-1,"dirtLevel":0.1,"modTurbo":false,"tyreSmokeColor":[255,255,255],"plate":"ZENU0864","modArmor":-1,
"modOrnaments":-1,"modTrimB":-1,"modEngineBlock":-1,"color2":111,"modLivery":-1,"modRoof":-1,"modDoorSpeaker":-1,"wheelColor":156,"modFrontWheels":-1,"modTransmission":-1,
"windowTint":-1,"modSpeakers":-1}

]]--

-- Spawn Cars
function SpawnVehicle(vehicle, plate)
	print("SpawnVehicle", json.encode(vehicle), plate)
	...
	TriggerServerEvent('esx_advancedgarage:setVehicleName', plate, name)
end
		...
		if Config.UseCarGarages then
			for k,v in pairs(Config.CarGarages) do
				if (GetDistanceBetweenCoords(coords, v.GaragePoint.x, v.GaragePoint.y, v.GaragePoint.z, true) < Config.DrawDistance) then
					canSleep = false
					DrawMarker(27, v.GaragePoint.x, v.GaragePoint.y, v.GaragePoint.z + 0.1 , 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 0, 25, 165, 165, 0,0, 0,0)

					DrawMarker(27, v.DeletePoint.x, v.DeletePoint.y, v.DeletePoint.z + 0.1 , 0, 0, 0, 0, 0, 0, 1.6001, 1.6001, 1.5001, 0, 25, 165, 165, 0,0, 0,0)
					break				
				end
			end
			...
			for k,v in pairs(Config.CarPounds) do
				if (GetDistanceBetweenCoords(coords, v.PoundPoint.x, v.PoundPoint.y, v.PoundPoint.z, true) < Config.DrawDistance) then
					canSleep = false
					DrawMarker(27, v.PoundPoint.x, v.PoundPoint.y, v.PoundPoint.z + 0.1 , 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 0, 25, 165, 165, 0,0, 0,0)
					break				
				end
			end
		end
