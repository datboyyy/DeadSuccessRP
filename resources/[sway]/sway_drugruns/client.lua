...
function CreateOxyVehicle()
	...
	TriggerEvent('persistent-vehicles/register-vehicle', oxyVehicle)
...
function CreateBlip()
    ...
    exports['mythic_notify']:SendAlert('inform', 'Ight you got the location now look at your map')
end
...
AddEventHandler("denoms", function(denomsp)
  ...
  hasdenoms = denomsp
  if hasdenoms then
	TriggerEvent('chat:addMessage', {template = '<div class="chat-message status stategained"><r> {0}</r> {1}</div>',args = {"STATUS: ", "Your currency has Multiple Denominations"}})
  else
	TriggerEvent('chat:addMessage', {template = '<div class="chat-message status stategained"><r> {0}</r> {1}</div>', args = {"STATUS: ", "Your currency no longer has Multiple Denominations"}})
  end
end)
...
local bandprice = 8000
local rollcashprice = 5000
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if HasStolenGoods() then
			...
			if math.random(10) == 1 then
				if OxyRun then
				cashPayment = math.random(150,250)	
				TriggerEvent( "player:receiveItem", "Gruppe6Card", 1 )
				TriggerEvent( "player:receiveItem", "decrypterenzo", 1 )
			...
			if math.random(49) == 49 then
				cashPayment = math.random(280,320)
			end
...
AddEventHandler("oxydelivery:client", function()
	...
	while tasking do
		...
		if toolong < 0 then
			...
			tasking = false
			OxyRun = false
			TriggerEvent('phone:addnotification', 'EMAIL',  "You are no longer selling Oxy due to taking too long to drop off! ")
		end
...
Citizen.CreateThread(function()
    ...
    while true do
		...
		if dropOff6 < 1.6 and not OxyRun then
			...
			if IsControlJustReleased(0,38) then
				ESX.TriggerServerCallback('startdealingjob', function(requiredmoney)
...
Citizen.CreateThread(function()
    ...
    while true do
		...
		elseif OxyRun then
			...
			if not DoesEntityExist(oxyVehicle) or GetVehicleEngineHealth(oxyVehicle) < 100.0 then
				OxyRun = false
				tasking = false
				TriggerEvent('phone:addnotification', 'EMAIL',  "Dude! You fucked the car up, I canceled your run, asshole! ")
			else
				if tasking then
			        Citizen.Wait(30000)
			    else
